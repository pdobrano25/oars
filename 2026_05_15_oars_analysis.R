...
#  
v.date = "2026_05_15" # this object gets saved to indicate latest version
# save.image("./2026_05_15_oars_analysis_messy_di.Renv") # contains all; LSARP and UC

# load("./2026_05_15_oars_analysis_messy_di.Renv")
# note: 2026_02_27 is a bare analysis; only this script and no defunct parts
...
# load de-identification codes
oars.deid = read.csv("./oars_plots_deidentified/2026_05_12_oars_codes.csv")
...
rs.colors.type = c("Potato" = "#F8766D", "Green banana" = "#A3A500", "Corn" = "#00BF7D", "Cross-linked" = "#00B0F6", "Cross-linked potato" = "#E76BF3")
...
# and new ones that include host protein
omics.colors
omics.colors.px = c("ASV" = "#8DD3C7",
                 "Species" = "#FFFFB3",
                 "Pathway" =  "#BEBADA",
                 "COG" = "#FB8072",
                 "CAZyme" = "#80B1D3",
...
doubleup = function(data = beta.trajectory.data,
  ...
  if(delta == TRUE){
    ...
    if(sum(colnames(data.036) == "s100a89")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.s100 = s100a89[reltiming == "post"] - s100a89[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.s100 = ((s100a89[reltiming == "pre"])))
    }
...
delta.omic.prepare = function(data = oars.mbx.annotated.mat,
  ...
  if(double == T){
    ...
    data = merge(data,
                          metadata.oars.stool[,c("standard.name", "HM","oars.days", "timing","phase", "diagnosis", "adj.fiber")], by=c("standard.name"))
  }  
  # fix colname order
...
# NOTE: unicode p does not render on ggplot2 in RStudio; but does render in cairo_pdf
# ALSO: somehow unicode p screws up the p value number that follows (repeats the first digit after .)
pval.positioner = function(
  ...
  data.frame(phase = c("RS", "Washout"),
             ...
             yval = y.val,
             label = c(paste("𝘱:", round(data.frame(data1)[2,5], digits=3)),
                       paste("𝘱:", round(data.frame(data2)[2,5], digits=3))))
  }else{
    data.frame(phase = c("RS", "Washout"),
               xval = c(rs.center, wo.center),
               yval = y.val,
...
metadata.oars.stool.plot = ggplot(metadata.oars.stool %>%
  ...
  geom_point(aes(shape=oars.on.rs, fill=oars.on.rs,
             ...
             size=3)+
  scale_fill_manual(labels = c("Baseline", "RS Intervention", "Washout"), values=c("grey", 2, "grey"))+
  scale_shape_manual(labels = c("Baseline", "RS Intervention", "Washout"), values=c(23, 21,21))+
  scale_alpha_manual(labels = c("Baseline", "RS Intervention", "Washout"), values=c(0.25,1))+
  guides(alpha= "none")+
  ...
  # deid
  scale_y_discrete(labels = oars.deid$code, 
                   breaks = oars.deid$HM) +
...
redcap.data.nutrients$standard.name = rownames(redcap.data.nutrients)

oars.nutrient.all.data = merge(redcap.data.nutrients,
                               metadata.oars.stool, by="standard.name", all.y=T) # keep all entries

# now loop through nutrients and assess changes over treatment / washout (energy-adjusted)

nutrients = colnames(redcap.data.nutrients)
nutrients = nutrients[!nutrients %in% c("standard.name", "alcohol")]

oars.nutrient.all.data.lmer = do.call(rbind, lapply(nutrients, function(x){
  #x = colnames(redcap.data.nutrients)[4]
  print(x)
  # subset to nutrient of interest
  data.subset = oars.nutrient.all.data[,colnames(oars.nutrient.all.data) %in% c(x, "energy", "HM", "oars.days","timing", "phase", "diagnosis")]
  # normalize
  if(x != "energy"){
  data.subset$variable = data.subset[,colnames(data.subset) == x] / data.subset$energy / 4.184 * 1000
  }
  if(x == "energy"){
    data.subset$variable = data.subset$energy / 4.184
  }
  # split by phase
  do.call(rbind, lapply(c("treatment", "washout"), function(y){
    if(y == "treatment"){
    data.subset = subset(data.subset, timing %in% c("0M", "3M", "6M"))
    }
    if(y == "washout"){ # essentially an "else" statement
      data.subset = subset(data.subset, timing %in% c("6M", "9M", "12M"))
    }
    # lmer
    lmer.output = lmerTest::lmer(scale(log10(variable)) ~ scale(oars.days) + diagnosis + (1|HM), data.subset)
    data.frame(nutrient = x,
               phase = y,
               coef = summary(lmer.output)$coef[2,1],
               pval = summary(lmer.output)$coef[2,5])
  }))
  }))

oars.nutrient.all.data.lmer.df = oars.nutrient.all.data.lmer %>%
  #arrange(pval) %>%
  mutate(padj = p.adjust(pval, method="BH"))

# nothing sig after FDR
# coef = Beta coefficient of log10-transformed, energy-adjusted nutrient value (or energy)
# not controlling for RS product energy

# save as table
oars.nutrient.all.data.lmer.df <- oars.nutrient.all.data.lmer.df %>%
  pivot_wider(
    names_from  = phase,
    values_from = c(coef, pval, padj),
    names_sep   = "."
  ) %>%
  # Optional: select and rename for exact column order/names
  select(
    nutrient,
    treatment.coef = coef.treatment,
    treatment.pval = pval.treatment,
    treatment.padj = padj.treatment,
    washout.coef   = coef.washout,
    washout.pval   = pval.washout,
    washout.padj   = padj.washout
  )

write.csv(oars.nutrient.all.data.lmer.df,
          "./oars_paper/tables/2026_02_09_supp_4_nutrients.csv")
...
metadata.oars.water.plot = ggplot(subset(metadata.oars.stool, compliant == TRUE),
 ...
 theme_classic()+theme(legend.position="none",
  ...
  geom_text(data=pval.positioner("stool_water_perc",
                                 ...
                                 stats.water.6912,
                                 yadjust = 1.03), 
            aes(x=xval, y=yval*100, label=label), vjust=1,size=4)+
...
metadata.oars.load.asv.plot = ggplot(subset(metadata.oars.stool.asv, compliant == TRUE),
  ...
  geom_text(data=pval.positioner("load.asv",
                                 ...
                                 stats.load.asv.6912,
                                 yadjust = 1.01), aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  labs(x="Days since starting RS", y=expression(Predicted~Microbial~Load~(Log[10])))
...
oars.stool.water.load.correlation.plot = ggplot(metadata.oars.stool.asv,
  ...
  facet_wrap(~"Stool Moisture vs Predicted Microbial Load")+
  labs(x="Stool Moisture (%)",
        y=expression(Predicted~Microbial~Load~(Log[10])))
...
metadata.oars.stool.asv.beta.between.plot = ggplot(subset(beta.trajectory.data,compliant==TRUE),
  ...
  geom_text(data=pval.positioner("between.beta",
                                 ...
                                 yadjust = 1.05), aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  labs(x="Days since starting RS", y="Bray-Curtis Dissimilarity")
...
oars.asv.maaslin.036.volcano = ggplot(subset(oars.asv.maaslin.036, value == "oars.days"),
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", feature), NA)), 
...
# // defunct, but needed for heatmap map
...
metadata.oars.starch.mucin.12.plot = ggplot(subset(metadata.oars.stool.asv, compliant == TRUE)%>%
  ...
  labs(x="Days", y="Starch:Mucin CAZyme")
...
t2 = Sys.time()
t2 - t1
oars.mpx.kegg.oars.pca.permanova
# treatment p drops from 0.008 to 0.020 when using PC1+PC2 only (but affected by set.seed variance)
# fiber p drops from 0.073 to 0.148
# HM p drops from 0.088 to 0.156
...
# treatment p drops from 0.055 to 0.106 when using PC1+PC2 only (but affected by set.seed variance)
# fiber p drops from 0.042 to 0.382
# HM p drops from 0.005 to 0.054
...
oars.mpx.cog.maaslin.036.volcano = ggplot(oars.mpx.cog.maaslin.036,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", feature), NA)), 
    size=2.5, max.overlaps=10)+  facet_wrap(~"Treatment: COG")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
...
# treatment p drops from 0.001 to 0.006 when using PC1+PC2 only (but affected by set.seed variance)
# fiber p drops from 0.063 to 0.249
# HM p increases from 0.162 to 0.024
...
oars.mpx.cazy.maaslin.036.volcano = ggplot(oars.mpx.cazy.maaslin.036,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", feature), NA)), 
    size=2.5, max.overlaps=10)+  facet_wrap(~"Treatment: CAZyme")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
...
oars.mpx.cazy.maaslin.6912.volcano = ggplot(oars.mpx.cazy.maaslin.6912,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", feature), NA)), 
    size=2.5, max.overlaps=10)+  facet_wrap(~"Washout: CAZyme")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
...
metadata.oars.stool.starch.plot = ggplot(subset(metadata.oars.stool.asv,compliant==TRUE),
  ...
  labs(x="Days since starting RS", y="Starch CAZyme")
...
metadata.oars.stool.mucin.plot = ggplot(subset(metadata.oars.stool.asv,compliant==TRUE),
  ...
  labs(x="Days since starting RS", y="Mucin CAZyme")
...
metadata.oars.stool.starch.mucin.plot = ggplot(subset(metadata.oars.stool.asv,compliant==TRUE),
  ...
  geom_text(data=pval.positioner("starch.mucin",
                                 ...
                                 yadjust = 1.15), aes(x=xval, y=yval, label=label), vjust=1,size=4)+
  labs(x="Days since starting RS", y="Starch:Mucin CAZyme")
...
t2 = Sys.time()
t2 - t1
oars.mbx.pca.permanova
# treatment p drops from 0.793 to 0.512 when using PC1+PC2 only (but affected by set.seed variance)
# fiber p changes from 0.070 to 0.052
# HM p increases from 0.082 to 0.039
...
oars.mbx.maaslin.036.volcano = ggplot(oars.mbx.maaslin.036,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\| .*", "", feature), NA)), 
    size=2.5, max.overlaps=10)+  facet_wrap(~"Treatment: Metabolite")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
...
oars.mbx.maaslin.6912.volcano = ggplot(oars.mbx.maaslin.6912,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\| .*", "", feature), NA)), 
    size=2.5, max.overlaps=10)+  facet_wrap(~"Washout: Metabolite")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
...
oars.pca.permanova = rbind(
  ...
  oars.mpx.cazy.pca.permanova%>% as.data.frame() %>% mutate(data.type = "CAZyme") %>% mutate(var = rownames(.)),
...
oars.pca.permanova.plot = ggplot(oars.pca.permanova %>% 
         mutate(data.type = factor(data.type, levels=c("ASV", "Species", "Pathway", "COG", "CAZyme", "Metabolite"))) %>%
...
oars.hysteresis = rbind(
  ...
  lmerTest::lmer(scale((between.beta)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(beta.trajectory.data, timing %in% c("0M", "3M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.))%>% mutate(time = "RS1", variable = "Bray-Curtis Dissimilarity") %>% subset(feature == "scale(oars.days)"),
  lmerTest::lmer(scale((between.beta)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(beta.trajectory.data, timing %in% c("3M", "6M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.))%>% mutate(time = "RS2", variable = "Bray-Curtis Dissimilarity") %>% subset(feature == "scale(oars.days)"),
  ...
  lmerTest::lmer(scale(log10(starch)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("0M", "3M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.))%>% mutate(time = "RS1", variable = "Starch CAZyme") %>%  subset(feature == "scale(oars.days)"),
  lmerTest::lmer(scale(log10(starch)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("3M", "6M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.)) %>% mutate(time = "RS2", variable = "Starch CAZyme") %>% subset(feature == "scale(oars.days)"),
  
  lmerTest::lmer(scale(log10(mucin)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("0M", "3M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.)) %>% mutate(time = "RS1", variable = "Mucin CAZyme") %>% subset(feature == "scale(oars.days)"),
  lmerTest::lmer(scale(log10(mucin)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("3M", "6M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.)) %>% mutate(time = "RS2", variable = "Mucin CAZyme") %>% subset(feature == "scale(oars.days)"),
  
  lmerTest::lmer(scale((starch.mucin)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("0M", "3M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.)) %>% mutate(time = "RS1", variable = "Starch:Mucin CAZyme") %>% subset(feature == "scale(oars.days)"),
  lmerTest::lmer(scale((starch.mucin)) ~ scale(oars.days) + diagnosis + adj.fiber + (1|HM),
                 subset(metadata.oars.stool.asv, timing %in% c("3M", "6M") & compliant == TRUE)) %>% summary() %>% coef() %>% as.data.frame() %>% dplyr::select(Estimate, `Pr(>|t|)`) %>% mutate(feature = rownames(.)) %>% mutate(time = "RS2", variable = "Starch:Mucin CAZyme") %>% subset(feature == "scale(oars.days)")
)
...
oars.hysteresis.plot = ggplot(oars.hysteresis %>% 
         ...
         mutate(variable = factor(variable, levels=c("Butyrogens (%)", "Fecal calprotectin (μg/g)", "ASV Richness", "Shannon Diversity",
                                                     "Functional Redundancy", "Bray-Curtis Dissimilarity", "Stool Moisture (%)",
                                                     "Microbial Load (Log10 Predicted)", "Starch CAZyme", "Mucin CAZyme", "Starch:Mucin CAZyme"))),
...
oars.gam.results = rbind(
  ...
  mutate(feature = factor(c("Butyrogens", "Fecal Calprotectin",
                      ...
                      "Bray-Curtis Dissimilarity", "Starch CAZyme", "Mucin CAZyme", "Starch:Mucin CAZyme"), levels=rev(c("Butyrogens", "Fecal Calprotectin",
                                                                                                           "Stool Moisture", "Microbial Load","Functional Redundancy",
...
oars.ph.score.change.plot = ggplot(metadata.oars.stool.double %>%
         ...
         mutate(ordering = sum(ordering)) %>%
           dplyr::select(code, ordering, Phase, Response) %>% distinct(),
       aes(x=as.factor(Phase), y=reorder(code, ordering)))+
  geom_text(aes(x = as.factor(Phase), y=reorder(code, ordering), label=code,
                color=ifelse(Phase == "RS 1", "black", "white")),
            hjust=1.4)+
  scale_color_manual(values=c("black", "white"))+
  geom_path(aes(group=code), linetype=2, alpha=0.5)+
  geom_point(shape=22, aes(fill = Response), size=5)+
...
# Are CD and UC randomly distributed among Strong vs Weak Fermenters?
metadata.oars.stool.double[,c("hm_phase", "diagnosis", "response")] %>% 
  distinct() %>%
  dplyr::select(diagnosis, response) %>%
  table() %>%
  fisher.test()

# And disease site?
metadata.oars.location = read.csv("./2026_02_04_oars_location.csv")
metadata.oars.stool.double[,c("HM", "hm_phase", "response")] %>%
  merge(metadata.oars.location, by="HM") %>%
  distinct() %>%
  dplyr::select(response, location) %>%
  #mutate(l1 = ifelse(location == "pancolitis", 1, 2)) %>%
  #dplyr::select(response, l1) %>%
  table() %>%
  fisher.test()
# visually confirm
metadata.oars.stool.double[,c("HM", "hm_phase", "response")] %>%
...
# and when stratifying by disease type?
stats.fcal.ph.low.cd = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + adj.fiber + (1|HM), 
                                   subset(metadata.oars.stool.double, response == "low" & diagnosis =="CD")) %>% summary() %>% coef()
stats.fcal.ph.low.uc = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + adj.fiber + (1|HM), 
                                      subset(metadata.oars.stool.double, response == "low"& diagnosis == "UC")) %>% summary() %>% coef()

stats.fcal.ph.high.cd = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + adj.fiber + (1|HM), 
                                    subset(metadata.oars.stool.double, response == "high"& diagnosis == "CD")) %>% summary() %>% coef()
stats.fcal.ph.high.uc = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + adj.fiber + (1|HM), 
                                       subset(metadata.oars.stool.double, response == "high"& diagnosis == "UC")) %>% summary() %>% coef()
# trends remain; 
# CD-Strong has n=5 and p = 0.078
# UC-Strong has n=3 and p = 0.26
# while weak are all p > 0.50 or slfc > 0.50

# try wilcoxon
do.call(rbind, lapply(c("CD", "UC"), function(x){
  do.call(rbind, lapply(c("high", "low"), function(y){
    # subset data
    data.subset = subset(metadata.oars.stool.double, response == y & diagnosis == x)
    # wilcox data
    data.subset = reshape2::acast(data.subset, hm_phase ~ reltiming, value.var="fcal") %>% as.data.frame()
    # wilcox
    wilcox.result = wilcox.test(log10(data.subset$pre),
                                log10(data.subset$post))
    data.frame(diagnosis = x,
               response = y,
               lfc = mean(log10(data.subset$post)) - mean(log10(data.subset$pre)),
...
do.call(rbind, lapply(c("high", "low"), function(y){
    # subset data
    data.subset = subset(metadata.oars.stool.double, response == y)
    # wilcox data
    data.subset = reshape2::acast(data.subset, hm_phase ~ reltiming, value.var="fcal") %>% as.data.frame()
    # wilcox
    wilcox.result = wilcox.test(log10(data.subset$pre),
...
metadata.oars.fcal.double.plot = ggplot(subset(metadata.oars.stool.double, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2")) %>%
  ...
  geom_text(data = stats.fcal.ph%>%
            ...
            aes(label=paste("𝘱:", round(pval, digits=3))), size=4.5)+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
...
metadata.oars.water.double.plot = ggplot(subset(metadata.oars.stool.double, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2")) %>%
  ...
  geom_text(data = stats.water.ph, x=1.5, y=(max(na.omit(metadata.oars.stool.double$stool_water_perc)))*100, 
            aes(label=paste("𝘱:", round(pval, digits=3))), size=3.5)+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=10),
...
metadata.oars.load.asv.double.plot = ggplot(subset(metadata.oars.stool.asv.double, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2")) %>%
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x="",
       y=expression(Predicted~Microbial~Load~(Log[10]))) # shortening title here so it fits in the plot
...
oars.asv.data.glom.lmer.volcano = ggplot(oars.asv.data.glom.lmer,
  ...
  scale_y_continuous(transform=neg_log10_trans,
                     breaks=c(0.05,0.1, 0.20, 0.5, 1))+
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", taxa), NA)), 
    size=2.5, max.overlaps=10)+
  scale_fill_gradient2(low="blue", high="red")+
...
oars.mpx.cazy.lmer.volcano = ggplot(oars.mpx.cazy.lmer,
  ...
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.20, gsub("\\..*", "", taxa), NA)), 
    size=2.5, max.overlaps=10)+  scale_fill_gradient2(low="blue", high="red")+
  facet_wrap(~"Interaction: CAZyme")+
  theme_classic()+theme(legend.position="none",
                        strip.text = element_text(size=10))+
  labs(x="Interaction Coefficient", y="FDR")
...
oars.mpx.cazy.lmer.split.volcano = ggplot(oars.mpx.cazy.lmer.split %>%
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x="Adjusted Coefficient", y="FDR", title="CAZyme")
...
oars.mpx.cazy.lmer.split.volcano.unadj = ggplot(oars.mpx.cazy.lmer.split %>%
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x="Adjusted Coefficient", y="p-value", title="CAZyme")
...
# Note: different from saved version (e.g. RS Type instead of RS Name)
                           ...
                           ifelse(feature %in% colnames(oars.mgx.taxa.filt.3), "Species",
                                  ifelse(feature %in% colnames(oars.mpx.kegg.filt.3), "Pathway",
                                         ifelse(feature %in% colnames(oars.mpx.cog.filt.3), "COG", 
                                                ifelse(feature %in% colnames(oars.mpx.cazy.filt.3), "CAZyme", 
...
# show bottom 20 and top 20
oars.double.response.tree.lm.data = oars.double.response.tree.lm %>%
  subset(slope != 0)%>%
  group_by(feature) %>%
  mutate(slope.sd = sd(slope))%>%
  arrange(slope.sd) %>%
  ...
  arrange(-slope.sd) %>%
  group_by(feature) %>%
  mutate(x.pos = min(slope)) %>%
  ungroup() %>%
  mutate(x.pos.factor = -sqrt(-x.pos)/2) %>%
  mutate(label.name = feature)

# create separate data.frame for names
oars.double.response.tree.lm.data.names = oars.double.response.tree.lm.data[,c("feature","x.pos","x.pos.factor",
                                                                               "label.name","slope.sd", "Datatype","stage")] %>% distinct() %>%
  arrange(-slope.sd)
# fix position
oars.double.response.tree.lm.data.names = oars.double.response.tree.lm.data.names %>%
  arrange(slope.sd) %>%
  mutate(x.pos.2 = accumulate(
    x.pos,
    ~ min(.x, .y),          # keep the larger of previous enforced value or current
    .init = Inf            # start low enough
  )[-1]) 

# replace duplicate feature name
oars.double.response.tree.lm.data.names$label.name[duplicated(oars.double.response.tree.lm.data.names$label.name)] <- NA
# bold features of interest
oars.double.response.tree.lm.data.names$bold.name = 
  ifelse(oars.double.response.tree.lm.data.names$label.name %in% c("Nicotinate and nicotinamide metabolism",
                                                             "Pantothenate and CoA biosynthesis",
                                                             "Vancomycin resistance",
                                                             "N-Stearoyl Valine "),
         oars.double.response.tree.lm.data.names$label.name, NA)
# remove bold feature from regular list
oars.double.response.tree.lm.data.names$label.name = 
  ifelse(!oars.double.response.tree.lm.data.names$label.name %in% c("Nicotinate and nicotinamide metabolism",
                                                             "Pantothenate and CoA biosynthesis",
                                                             "Vancomycin resistance",
                                                             "N-Stearoyl Valine "),
...
oars.double.response.tree.lm.plot = ggplot(oars.double.response.tree.lm.data,
                                           aes(x=slope, y=reorder(feature, -rank(slope.sd))))+
  geom_line(aes(group=feature, color=Datatype))+
  geom_path(aes(group=HM), linetype=2, linewidth=0.2, color="black", alpha=0.5)+
  geom_point(aes(shape=Datatype, fill=Datatype), size=2.5)+
  # add label
  geom_text(data=oars.double.response.tree.lm.data.names,
            aes(x=x.pos-0.3, y=reorder(feature, -rank(slope.sd)), label=label.name), 
            size=3, hjust=1)+
  # add bold label
  geom_text(data=oars.double.response.tree.lm.data.names,
            aes(x=x.pos-0.3, y=reorder(feature, -rank(slope.sd)), label=bold.name), 
  ...
  facet_wrap(~ stage, nrow=2, 
             scales="free",strip.position = "right")+
  labs(x="Patient Slope")
oars.double.response.tree.lm.plot
# this is it; just need to check for bugs
...
# filtered features
# built lmer with random slopes (treatment period, starch ~ feature + (feature | HM)
# "conserved" = top 20 lowest pvals (most consistent)
# "variable" = top 20 highest slope variance
...
# Host PX (note: generated in supplementary code; cleaning code would be onerous)
# subset to overlapping samples
oars.px.all.mat.2 = oars.px.all.df[rownames(oars.px.all.df) %in% metadata.oars.stool$standard.name,] %>% as.data.frame()
dim(oars.px.all.mat.2) # 66
oars.px.all.mat.2.pa = oars.px.all.mat.2
oars.px.all.mat.2.pa[oars.px.all.mat.2.pa!=0] = 1
oars.px.all.mat.3 = oars.px.all.mat.2[,colSums(oars.px.all.mat.2.pa) >= nrow(oars.px.all.mat.2)*0.8]
dim(oars.px.all.mat.3) # 295
...
oars.fecalcal.omics.data = merge(oars.fecalcal.omics.data,
                                 ...
                                 by="Row.names")
oars.fecalcal.omics.data = merge(oars.fecalcal.omics.data,
                                 oars.px.all.mat.2%>%as.data.frame()%>%
                                   mutate(`Row.names` = rownames(.)),
                                 by="Row.names")
dim(oars.fecalcal.omics.data) # 59
...
dim(oars.fecalcal.omics.data)
# 2712 features (3149 with host px)
# 59 samples with matching omics data (and 59 with host px)
...
# 115
subset(metadata.oars.stool.omics.cor.bh, feature == "Bifidobacterium_adolescentis")
# excellent
...
# add color/shape
metadata.oars.stool.omics.cor.bh = metadata.oars.stool.omics.cor.bh %>%
  mutate(omic.type = ifelse(feature %in% colnames(oars.asv.data.glom.filt.2), "ASV",
                            ifelse(feature %in% colnames(oars.mgx.taxa.filt.2), "Species",
                                   ifelse(feature %in% colnames(oars.mpx.kegg.filt.2), "Pathway",
                                          ifelse(feature %in% colnames(oars.mpx.cog.filt.2), "COG",
                                                 ifelse(feature %in% colnames(oars.mpx.cazy.filt.2), "CAZyme",
                                                        ifelse(feature %in% colnames(oars.mbx.filt.2), "Metabolite", 
                                                               ifelse(feature %in% colnames(oars.px.all.mat.2), "Host protein", "fCal"))))))))
metadata.oars.stool.omics.cor.bh$omic.type = factor(metadata.oars.stool.omics.cor.bh$omic.type, levels=names(omics.colors.px))

# shorten name
metadata.oars.stool.omics.cor.bh$clean.feature = ifelse(nchar(as.character(metadata.oars.stool.omics.cor.bh$feature))>30, paste(substr(metadata.oars.stool.omics.cor.bh$feature, 1, 30), "...", sep=""), as.character(metadata.oars.stool.omics.cor.bh$feature))

# position labels; B. adolescentis is #31 and padj 0.14

subset(metadata.oars.stool.omics.cor.bh, padj < 0.20)

### algorithmically position labels
label_df = metadata.oars.stool.omics.cor.bh %>%
  subset(padj < 0.1 | feature == "Bifidobacterium_adolescentis") %>%
  arrange(desc(-log10(padj)))  # or any custom sort

calculate_arc_positions <- function(data = label_df, 
                                    n_points) {
  if (nrow(data) == 0) return(data.frame(x = numeric(), y = numeric(), label = character()))
  
  # Extract x (cor) and y (neg_log10_padj)
  x <- data$value
  y <- (data$padj)
  
  # Generate equally spaced x-coordinates for labels
  n <- nrow(data)
  
  # create new arrangement based on value * padj
  data = data %>% 
    arrange(padj) %>%
    #group_by(padj) %>%
    arrange(padj)  %>%
  ...
  # calculate x after splitting by direction
  x_labels <- ifelse(data$value >=0, 
                     data$value-0.15, 
                     data$value+0.15)
  
  # Calculate y-coordinates using the quadratic fit
  y_labels = 10^-seq(from = -log10(0.001), 
                     to =  -log10(0.16), 
                     length=nrow(data))
  
  return(data.frame(x = x_labels,
                    y = y_labels, 
                    feature = data$feature))
}

# Calculate label positions (+ and - separately)
label_positions <- calculate_arc_positions(label_df, n_points = nrow(label_df))

oars.fcal.cor.data = merge(label_positions, metadata.oars.stool.omics.cor.bh, by="feature")

# important labels
fcal.cor.labels = c("Bifidobacterium_adolescentis",
                    "Magnesium-transporting ATPase ...",
                   "Holdemania_filiformis",
                   "Flagellar motor switch/type II...",
                   "Flagellar biosynthesis protein...",
                   "UDP-N-acetylmuramyl pentapepti...")

oars.fecalcal.omics.cor.plot = ggplot(metadata.oars.stool.omics.cor.bh,
                                      aes(x=value, y=(padj)))+
  # line to name (neg)
  geom_segment(data=subset(oars.fcal.cor.data, value<0),
               aes(x=x-0.01,xend= value, y=y,yend=(padj), 
                   color=omic.type),
               linetype=1)+
  # line to name (pos)
  geom_segment(data=subset(oars.fcal.cor.data, value>=0),
               aes(x=x+0.01,xend= value, y=y,yend=(padj), 
                   color=omic.type),
               linetype=1)+
  geom_point(data=subset(metadata.oars.stool.omics.cor.bh, padj >= 0.20),
             shape=21, size=2.5, fill="darkgrey")+
  geom_point(data=subset(metadata.oars.stool.omics.cor.bh, padj < 0.20),
             size=2.5, aes(fill=omic.type, shape=omic.type))+
  # feature labels (neg)
  geom_text(data=subset(oars.fcal.cor.data, value<0)%>%
              # bold font to indicate C panel
              mutate(kegg = ifelse(clean.feature %in% fcal.cor.labels, "bold", "plain")), 
            aes(x=x, y=(y), label=clean.feature,
                fontface = kegg), size=2.2, hjust=0)+ 
  # feature labels (pos)
  geom_text(data=subset(oars.fcal.cor.data, value>=0)%>%
              # bold font to indicate C panel
              mutate(clean.feature = gsub("S10A", "S100A", clean.feature)) %>%
              mutate(kegg = ifelse(clean.feature %in% fcal.cor.labels, "bold", "plain")), 
            aes(x=x, y=(y), label=clean.feature,
                fontface = kegg), size=2.2, hjust=1)+ 
  scale_fill_manual(values=c(omics.colors.px))+
  scale_color_manual(values=c(gsub("#FFFFB3", "#FFEEA9", omics.colors.px)))+
  scale_shape_manual(values=omics.shapes.px)+
  scale_y_continuous(transform=neg_log10_trans,
                     breaks=c(0.01, 0.05,0.1, 0.20, 0.5, 1))+
  #scale_fill_manual(values=c("black", "white"))+
  geom_hline(yintercept=0.20, linetype=2, alpha=0.5)+
  guides(color = FALSE)+
  #facet_wrap(~"fCal Correlation")+
...
# this broke after adding host proteins; just focus on network instead
...
# and save this as a supplemental:
colnames(metadata.oars.stool.omics.cor.bh)


oars.fcal.cor.data = metadata.oars.stool.omics.cor.bh[,c("feature", "value", "pval", "padj", "omic.type", "clean.feature")]

oars.all.cor.data = metadata.oars.stool.omics.cor.df %>%
  mutate(padj = p.adjust(pval, method="BH")) %>%
  arrange(-abs(value))

# :: scatter plots --------------------------------------------------------

oars.fcal.cor.plot.1 = ggplot(subset(oars.fecalcal.omics.cor.df,
                                     clean.feature == "Bifidobacterium_adolescentis") %>%
                                mutate(clean.feature = gsub("_", " ", clean.feature)),
       aes(x=value,
           y=fcal))+
  geom_point(size = 2, shape=21, fill="black", color="white") +
  scale_shape_manual(values=omics.shapes)+
  guides(fill=FALSE, shape=FALSE, size = FALSE)+
  # colors: 
  scale_fill_manual(values=omics.colors)+
  ...
  #scale_y_log10()+
  #scale_x_log10()+
  theme_classic()+theme(strip.text=element_text(size=8, face="italic"))+
  #facet_wrap(~clean.feature, scales="free", nrow=2)+
  labs(x=bquote(italic(.("B. adolescentis")) ~ "Log"[2]*"FC"),
       y=expression(fCal~Log[2]*FC))

oars.fcal.cor.plot.2 = ggplot(oars.fecalcal.omics.lfc,
                              aes(x=`Flagellar motor switch/type III secretory pathway protein FliN`,
                                  y=`Roseburia_faecis`))+
  geom_point(size = 2, fill="black", shape=21, color="white") +
  #scale_shape_manual(values=omics.shapes)+
  guides(fill=FALSE, shape=FALSE, size = FALSE)+
  # colors: 
  #scale_fill_manual(values=omics.colors)+
  geom_smooth(method="lm", color="black")+
  ggpubr::stat_cor(aes(label = ..r.label..), method="spearman", size=3, 
                   label.y.npc=1, #vjust=1.5,
                   label.x.npc=0.5, hjust=0.5)+
  #scale_y_log10()+
  #scale_x_log10()+
  theme_classic()+theme(strip.text=element_text(size=8, face="italic"))+
  #facet_wrap(~clean.feature, scales="free", nrow=2)+
  labs(x=expression(FliN~Log[2]*FC),
       y=bquote(italic(.("R. faecis")) ~ "Log"[2]*"FC"))

oars.fcal.cor.plot.3 = ggplot(oars.fecalcal.omics.lfc,
                              aes(x=`Flagellar biosynthesis protein FliR`,
                                  y=`Bittarella_massiliensis`))+
  geom_point(size = 2, fill="black", shape=21, color="white") +
  #scale_shape_manual(values=omics.shapes)+
  guides(fill=FALSE, shape=FALSE, size = FALSE)+
  # colors: 
  ...
  theme_classic()+theme(strip.text=element_text(size=8, face="italic"))+
  #facet_wrap(~clean.feature, scales="free", nrow=2)+
  labs(x=expression(FliR~Log[2]*FC),
       y=bquote(italic(.("B. massiliensis")) ~ "Log"[2]*"FC"))

# 50% prevalence across ALL samples with matching omics (n=58)
oars.fecalcal.omics.lfc
...
# Host proteins
oars.host.mpx.lfc = oars.px.all.mat.3 %>% as.matrix() %>% reshape2::melt() %>%
  mutate(value = value+min(value[value!=0])/2)%>%
  rename("standard.name" = "Var1", "variable" = "Var2") %>%
  merge(metadata.oars.stool.double[,c("standard.name", "phase", "HM", "reltiming", "timing", "response")])%>%
  group_by(HM, phase, variable) %>%
                                         ...
                                         ifelse(variable %in% oars.mpx.cog.lfc$variable, "COG", 
                                                ifelse(variable %in% oars.mpx.cazy.lfc$variable, "CAZy",
                                                       ifelse(variable %in% oars.mbx.lfc$variable, "Metabolite",
                                                              ifelse(variable %in% oars.host.mpx.lfc$variable, "Host protein",
                                                              "ASV"))))))) %>% data.frame()
...
pheatmap::pheatmap(reshape2::acast(subset(oars.asv.mgx.mpx.mbx.lfc %>% merge(oars.deid) %>% mutate(sample = paste(code, timing)), feature %in% rownames(oars.asv.mgx.mpx.mbx.lfc.map.vimp)),
                   ...
                   fontsize_col = 8,
                   border_color="white",
                   labels_row = make_bold_names(reshape2::acast(subset(oars.asv.mgx.mpx.mbx.lfc, feature %in% rownames(oars.asv.mgx.mpx.mbx.lfc.map.vimp)),
                                                                feature ~ sample, value.var="slfc"),
                                                rownames, c("Bifidobacterium_adolescentis",
                                                            "Magnesium-transporting ATPase (P-type)")),
                   annotation_col = oars.omics.lfc.map %>%
                     tidyr::separate(sample, into=c("HM", "timing"), sep="_") %>%
                     merge(oars.deid) %>%
                     mutate(sample = paste(code, timing)) %>% 
                     column_to_rownames(var="sample") %>% select(-HM, -code, -timing),
                   ...
                   annotation_row = oars.asv.mgx.mpx.mbx.lfc.map.vimp %>%
                     mutate(`fCal Correlation` = `Fcal Correlation`) %>%
                     dplyr::select(-`Fcal Correlation`),
...
# **(BH was applied to address the hypothesis of correlations with fcal)**
...
all.features.cor.all = metadata.oars.stool.omics.cor.df %>%
  # this is a second application of BH
  # **(BH was applied again to address the hypothesis of inter-feature correlations)**
  mutate(padj = p.adjust(pval, method="BH")) 
...
adj_matrix.umap = #metadata.oars.stool.omics.cor.sig %>% # or regularized with adj_matrix.df
  all.features.cor %>%
  # optional: force fcal to be positive
  mutate(value = ifelse(Var1 == "fcal", abs(value), value)) %>%
  # subset to strong
  #subset(abs(value) > 0.5) %>%
                            ...
                            ifelse(feature %in% colnames(oars.mpx.kegg.filt.2), "Pathway", 
                                   ifelse(feature %in% colnames(oars.mpx.cog.filt.2), "COG", 
                                          ifelse(feature %in% colnames(oars.mpx.cazy.filt.2), "CAZy", 
                                                 ifelse(feature %in% colnames(oars.mbx.filt.2), "Metabolite",
                                                        ifelse(feature %in% colnames(oars.asv.data.glom.filt.2), "ASV",
                                                               ifelse(feature %in% colnames(oars.px.all.mat.2),"Host protein", "Fecal calprotectin"))))))))
...
adj_matrix.umap.df$data.type = factor(adj_matrix.umap.df$data.type, levels=c("Fecal calprotectin", "ASV", "Species", "Pathway", "COG", "CAZy", "Metabolite", "Host protein")) 
...
metadata.oars.stool.omics.cor.network.pcoa = ggplot(adj_matrix.umap.df %>%
  ...
  geom_segment(aes(x=V1, xend=V1.B, y=V2, yend=V2.B, color = (cor), 
                   size = abs(cor), alpha =  abs(cor))) +  # Edge color by correlation, size by |corr|
  ...
  scale_shape_manual(values=c("Fecal calprotectin" = 21, omics.shapes.px))+
  guides(shape = FALSE, fill=FALSE, size = FALSE, alpha=FALSE)+
  scale_fill_manual(values= c("Fecal calprotectin" = "white", omics.colors.px))+
  #ggnetwork::geom_nodetext_repel(aes(label = ifelse(feature %in% c(label_df$feature, "Fecal calprotectin"), short, NA)), 
  #                    size = 3, max.overlaps=1000) +  # Node labels
...
# now clean up label placements
calculate_net_positions = function(data = adj_matrix.umap.df){
  ...
  # center value
  center = subset(data, feature == "Fecal calprotectin")[,c("V1", "V2")] %>% distinct()
  
  # cor data
  cor.data = subset(data, Var2 %in% label_df$feature & feature == "Fecal calprotectin")[,c("Var2","cor")] %>% distinct()
  colnames(cor.data)[1] = "feature"
  
  # position data
  data = subset(data, feature %in% label_df$feature)[,c("feature", "V1", "V2", "short")] %>% distinct()
  
  # merge
  data = merge(data, cor.data, by="feature")
  
  # arrange by Y axis
  data = data %>% arrange(-V2)
  
  # sign
  data = data %>% mutate(sign(cor))
  ...
  # calculate y after splitting by direction
  min_max_scale <- function(x, new_min = old.min, new_max = old.max) {
    (x - min(x)) / (max(x) - min(x)) * (new_max - new_min) + new_min
  }
  
  y_labels <- min_max_scale(rank(data$V2))
  
  return(data.frame(x = center$V1,
                    y = y_labels, 
                    x_point = data$V1,
                    y_point = data$V2,
                    sign = data$sign,
...
adj_matrix.umap.df.labels = calculate_net_positions()


metadata.oars.stool.omics.cor.network.pcoa.2 = ggplot(adj_matrix.umap.df %>%
                                                      subset(padj < 0.2), 
                                                    aes(V1, V2))+
  geom_segment(aes(x=V1, xend=V1.B, y=V2, yend=V2.B, color = (cor), 
                   size = abs(cor)), alpha =  0.2) +  # Edge color by correlation, size by |corr|
 # geom_segment(data = adj_matrix.umap.df.labels, 
#               aes(x=ifelse(sign==1, x-0.21, x+0.26),
#                   y=y, 
 #                  xend = x_point,
#                   yend = y_point), linetype=2, linewidth=0.2)+
  geom_point(size = 4, aes(shape=data.type), fill="white", color = "white", alpha=1) +
  geom_point(size = 4, aes(shape=data.type, fill=ifelse(short %in% adj_matrix.umap.df.labels$feature, "A", "B")), color = "black", alpha=0.9) +
  scale_shape_manual(values=c("Fecal calprotectin" = 21, omics.shapes.px))+
 # guides(shape = F, fill=F, size = FALSE)+
  guides(size = FALSE)+
  
  #scale_fill_manual(values= c("Fecal calprotectin" = "white", omics.colors.px))+
  #geom_text(data = adj_matrix.umap.df.labels, 
  #          aes(x=x_point,
  #              y=y_point, label=feature),
  #          size=2)+
  ggrepel::geom_text_repel(data=adj_matrix.umap.df.labels,
    aes(label=feature, x=x_point, y=y_point),
        #ifelse(feature == "Fecal calprotectin", feature, NA)),
            size=3, 
    max.overlaps=10,
    max.iter = 1000000,
    force=60,
   force_pull = 1,
    ...
    box.padding=0.1, point.padding=0,
    segment.size=0.2, segment.linetype=1)+
  ggnetwork::geom_nodetext_repel(aes(label=feature),
                           size=3, 
                           max.overlaps=10,
                           max.iter = 1000000,
                           force=50,
  ...
  labs(color = "Correlation",
       fill="Data type")
metadata.oars.stool.omics.cor.network.pcoa.2
...
oars.badol.starch.fcal.plot = ggplot(data=oars.badol.starch.fcal.plot.data %>%
  ...
  geom_rect(data=subset(oars.badol.starch.fcal.plot.data %>%
                          merge(oars.deid), timing == "0M"), 
            aes(xmin=0, xmax=max.days.1, 
                    fill=(response.1)),na.rm = TRUE,
           ymin=-Inf, ymax=Inf, alpha=0.5, color="white") +
  geom_rect(data=subset(oars.badol.starch.fcal.plot.data %>%
                          merge(oars.deid), timing == "3M"), 
  ...
  labs(x="Days since starting RS",
       y=expression(Scaled~Log[10]~Abundance))+
  facet_grid(code~.)
...
# add starch cazyme
oars.badol.simple.data = merge(oars.badol.simple.data,
                               metadata.oars.stool.asv.double[,c("hm_phase", "starch", "reltiming")], by=c("hm_phase", "reltiming"))
# check corr
plot(subset(oars.badol.simple.data, reltiming=="pre" & response=="high")$Bifidobacterium_adolescentis,
         subset(oars.badol.simple.data, reltiming=="pre" & response=="high")$starch)
# not compelling
...
oars.badol.simple.plot = ggplot(subset(oars.badol.simple.data, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2")) %>%
 ...
 # ggrepel::geom_label_repel(aes(label=RS_Name),size=2)+
  ...
  geom_text(data = stats.badol.ph %>%
            ...
            aes(label=paste("𝘱:", round(pval, digits=3))), size=3)+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=10),
                        strip.text = element_text(size=10),
...
subset(all.features.cor.all, grepl("Magnesium", Var1) & grepl("adolesc", Var2)) 
...
mg.badol = oars.mgx.taxa.filt.1 %>% as.data.frame() %>%
  ...
  merge(as.data.frame(redcap.data.nutrients %>%
                        mutate(standard.name = rownames(.)) %>%
                        dplyr::select(standard.name, magnesium, starch, energy)), by="standard.name") %>%
  #merge(metadata.oars.stool.asv.double[,c("standard.name", "HM")] %>% distinct(), by="standard.name") %>%
  mutate(mg.adj = magnesium / energy * 4.184 * 1000) # mg / kcal

mg.badol.lfc = mg.badol %>%
  mutate(HM = substr(standard.name, 1, 6),
         stl = substr(standard.name, 8,13)) %>%
  subset(!is.na(magnesium))%>%
  arrange(stl) %>%
  group_by(HM) %>%
  mutate(ba.lfc = log2(Bifidobacterium_adolescentis+1e-05)-lag(log2(Bifidobacterium_adolescentis+1e-05)),
         mg.lfc = (magnesium) - lag((magnesium)),
         amg.lfc = log2(mg.adj) - lag(log2(mg.adj)),
         amg.delta = (mg.adj) - lag((mg.adj))) %>%
  mutate(ba.dir =  sign(ba.lfc),
         mg.dir =  sign(mg.lfc),
         amg.dir = sign(amg.lfc)) %>%
  arrange(HM)


mg.badol.plot = ggplot(mg.badol.lfc %>% subset(!is.na(ba.lfc)),
                       aes(x=ba.lfc, y=amg.delta))+
  #geom_line(aes(group = HM), linewidth=0.2, linetype=2, alpha=0.7)+
  geom_point( fill="black", color="white", shape=21, size=2.5)+
  geom_smooth(method="lm", color="black")+
  #ggrepel::geom_text_repel(aes(label=standard.name), size=3)+
  ggpubr::stat_cor(method="spearman",
                   label.x.npc = "center",
                   label.y.npc = 1, hjust=0.5, vjust=-0.5)+
  theme_classic() + theme(legend.position="none")+
  labs(x = bquote(italic(.("B. adolescentis")) ~ "Log"[2]*"FC"),
       y =  "Δ Mg intake (mg/d / 1000 kcal)")
mg.badol.plot
...
# :: Simpler models - Group --------------------------------------------------------

# fcal
stats.fcal.036.simple = lmerTest::lmer(scale(log10(fcal)) ~ scale(oars.days) + (1|HM),
                                subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.fcal.6912.simple = lmerTest::lmer(scale(log10(fcal)) ~ scale(oars.days) + (1|HM),
                                 subset(metadata.oars.stool, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary()  %>% coef()

# Stool water
stats.water.036.simple = lmerTest::lmer(scale(log10(stool_water_perc)) ~ scale(oars.days) + (1|HM),
                                 subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.water.6912.simple = lmerTest::lmer(scale(log10(stool_water_perc)) ~ scale(oars.days) + (1|HM),
                                  subset(metadata.oars.stool, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary() %>% coef()

# Bacterial load
stats.load.asv.036.simple = lmerTest::lmer(scale(load.asv) ~ scale(oars.days) + (1|HM),
                                    subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.load.asv.6912.simple = lmerTest::lmer(scale(load.asv) ~ scale(oars.days) + (1|HM),
                                     subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary() %>% coef()

# Richness
stats.richness.036.simple = lmerTest::lmer(scale(log10(richness)) ~ scale(oars.days) + (1|HM),
                                    subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.richness.6912.simple = lmerTest::lmer(scale(log10(richness)) ~ scale(oars.days) + (1|HM),
                                     subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary() %>% coef()

# Shannon
stats.shannon.036.simple = lmerTest::lmer(scale(log10(shannon)) ~ scale(oars.days) + (1|HM),
                                           subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.shannon.6912.simple = lmerTest::lmer(scale(log10(shannon)) ~ scale(oars.days) + (1|HM),
                                            subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary() %>% coef()

# Functional redundancy
stats.fd.036.simple = lmerTest::lmer(scale((fd)) ~ scale(oars.days) + (1|HM),
                              subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M")& compliant == TRUE)) %>% summary() %>% coef()

stats.fd.6912.simple = lmerTest::lmer(scale((fd)) ~ scale(oars.days) + (1|HM),
                               subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M")& compliant == TRUE)) %>% summary() %>% coef()

# Butyrogens
stats.but.i.036.simple = lmerTest::lmer(scale(log10(but.i)) ~ scale(oars.days) + (1|HM),
                                 subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant == TRUE)) %>% summary() %>% coef()

stats.but.i.6912.simple = lmerTest::lmer(scale(log10(but.i)) ~ scale(oars.days) + (1|HM),
                                  subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M") & compliant == TRUE)) %>% summary() %>% coef()

# Beta
stats.beta.between.036.simple = lmerTest::lmer(scale((between.beta)) ~ scale(oars.days) + (1|HM),
                                        subset(beta.trajectory.data, timing %in% c("0M", "3M", "6M")& 
                                                 compliant == TRUE)) %>% summary() %>% coef()

stats.beta.between.6912.simple = lmerTest::lmer(scale((between.beta)) ~ scale(oars.days) + (1|HM),
                                         subset(beta.trajectory.data, timing %in% c("6M", "9M", "12M")& 
                                                  compliant == TRUE)) %>% summary() %>% coef()

# Starch
stats.starch.036.simple = lmerTest::lmer(scale(log2(starch)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                  subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M")& 
                                           compliant == TRUE)) %>% summary() %>% coef()

stats.starch.6912.simple = lmerTest::lmer(scale(log2(starch)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                   subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M")& 
                                            compliant == TRUE)) %>% summary() %>% coef()

# Mucin
stats.mucin.036.simple = lmerTest::lmer(scale(log2(mucin)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                         subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M")& 
                                                  compliant == TRUE)) %>% summary() %>% coef()

stats.mucin.6912.simple = lmerTest::lmer(scale(log2(mucin)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                          subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M")& 
                                                   compliant == TRUE)) %>% summary() %>% coef()

# Starch:Mucin
stats.starch.mucin.036.simple = lmerTest::lmer(scale((starch.mucin)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                        subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M")& 
                                                 compliant == TRUE)) %>% summary() %>% coef()

stats.starch.mucin.6912.simple = lmerTest::lmer(scale((mucin)) ~ scale(oars.days) + (1|HM) + (1|plate),
                                         subset(metadata.oars.stool.asv, timing %in% c("6M", "9M", "12M")& 
                                                  compliant == TRUE)) %>% summary() %>% coef()

# :: Simpler models - Response --------------------------------------------------------

# fcal
stats.fcal.ph.low.simple = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + (1|HM), 
                                   subset(metadata.oars.stool.double, response == "low")) %>% summary() %>% coef()
stats.fcal.ph.high.simple = lmerTest::lmer(scale(log10(fcal)) ~ reltiming + (1|HM), 
                                    subset(metadata.oars.stool.double, response == "high")) %>% summary() %>% coef()

# Stool water
stats.water.ph.low.simple = lmerTest::lmer(scale(log10(stool_water_perc)) ~ reltiming +  (1|HM), 
                                    subset(metadata.oars.stool.double, response == "low")) %>% summary() %>% coef()
stats.water.ph.high.simple = lmerTest::lmer(scale(log10(stool_water_perc)) ~ reltiming + (1|HM), 
                                     subset(metadata.oars.stool.double, response == "high")) %>% summary() %>% coef()

# Bacterial load
stats.load.asv.ph.low.simple = lmerTest::lmer(scale(load.asv) ~ reltiming + (1|HM), 
                                       subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.load.asv.ph.high.simple = lmerTest::lmer(scale(load.asv) ~ reltiming + (1|HM), 
                                        subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Richness
stats.richness.ph.low.simple = lmerTest::lmer(scale(log10(richness)) ~ reltiming + (1|HM), 
                                       subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.richness.ph.high.simple = lmerTest::lmer(scale(log10(richness)) ~ reltiming + (1|HM), 
                                        subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Shannon
stats.shannon.ph.low.simple = lmerTest::lmer(scale(shannon) ~ reltiming + (1|HM), 
                                      subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.shannon.ph.high.simple = lmerTest::lmer(scale(shannon) ~ reltiming + (1|HM), 
                                       subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Functional redundancy
stats.fd.ph.low.simple = lmerTest::lmer(scale(fd) ~ reltiming + (1|HM), 
                                 subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.fd.ph.high.simple = lmerTest::lmer(scale(fd) ~ reltiming + (1|HM), 
                                  subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Butyrogens
stats.but.i.ph.low.simple = lmerTest::lmer(scale(log10(but.i)) ~ reltiming + (1|HM), 
                                    subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.but.i.ph.high.simple = lmerTest::lmer(scale(log10(but.i)) ~ reltiming + (1|HM), 
                                     subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Beta
stats.beta.between.ph.low.simple = lmerTest::lmer(scale((between.beta)) ~ reltiming + (1|HM), 
                                           subset(beta.trajectory.data.double, response == "low")) %>% summary() %>% coef()
stats.beta.between.ph.high.simple = lmerTest::lmer(scale(between.beta) ~ reltiming + (1|HM), 
                                            subset(beta.trajectory.data.double, response == "high")) %>% summary() %>% coef()

# Starch
stats.starch.ph.low.simple = lmerTest::lmer(scale(log10(starch)) ~ reltiming + (1|HM) + (1|plate), 
                                     subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.starch.ph.high.simple = lmerTest::lmer(scale(log10(starch)) ~ reltiming + (1|HM) + (1|plate),
                                      subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Mucin
stats.mucin.ph.low.simple = lmerTest::lmer(scale(log10(mucin)) ~ reltiming + (1|HM) + (1|plate), 
                                            subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.mucin.ph.high.simple = lmerTest::lmer(scale(log10(mucin)) ~ reltiming + (1|HM) + (1|plate),
                                             subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()

# Starch:Mucin
stats.starch.mucin.ph.low.simple = lmerTest::lmer(scale((starch.mucin)) ~ reltiming + (1|HM) + (1|plate), 
                                           subset(metadata.oars.stool.asv.double, response == "low")) %>% summary() %>% coef()
stats.starch.mucin.ph.high.simple = lmerTest::lmer(scale((starch.mucin)) ~ reltiming + (1|HM) + (1|plate),
                                            subset(metadata.oars.stool.asv.double, response == "high")) %>% summary() %>% coef()
...
oars.px = read.csv("~/Downloads/results.pg_matrix.tsv", sep="\t")

# :: Unsupervised ---------------------------------------------------------

oars.px.all = oars.px

colnames(oars.px.all) = gsub("E..searches.Peter_rawstool_human.dia.", "", colnames(oars.px.all))
colnames(oars.px.all) = gsub("\\.raw.dia", "", colnames(oars.px.all))
colnames(oars.px.all) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_StintziLab_", "", colnames(oars.px.all))
colnames(oars.px.all) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_Stintzi_", "", colnames(oars.px.all))
oars.px.all = oars.px.all[,!grepl("QC", colnames(oars.px.all))]
colnames(oars.px.all) = gsub("\\_", "-", colnames((oars.px.all)))

# collapse to Protein.Name

# replace unannotated Genes with "unannotated"
oars.px.all$Protein.Names = ifelse(oars.px.all$Protein.Names == "", "unannotated", oars.px.all$Protein.Names)

# sum up protein intensities of the same Protein.Name name
oars.px.all.df = reshape2::melt(oars.px.all[,-c(1,3:6)])
oars.px.all.df = oars.px.all.df %>%
...
oars.px.all.df = reshape2::acast(oars.px.all.df, variable ~ Protein.Names, value.var="value")
dim(oars.px.all.df)

# replace NA with 0
oars.px.all.df[is.na(oars.px.all.df)] <- 0

# filter + process (no log)

# remove non-compliant
oars.px.all.mat.1 = oars.px.all.df[!rownames(oars.px.all.df) %in% c(non.compliant, "HM0924-STL-12"),] %>% as.data.frame()
oars.px.all.mat.1 = oars.px.all.mat.1[rownames(oars.px.all.mat.1) %in% metadata.oars.stool.mpx$standard.name,] %>% as.data.frame()

# filter to 10% prevalence
dim(oars.px.all.mat.1) # 4484 proteins, 49 samples
oars.px.all.mat.filt.1 = oars.px.all.mat.1
oars.px.all.mat.filt.1[is.na(oars.px.all.mat.filt.1)] = 0
oars.px.all.mat.filt.1[oars.px.all.mat.filt.1!=0] = 1
oars.px.all.mat.filt.1 = oars.px.all.mat.1[,colSums(oars.px.all.mat.filt.1) >= nrow(oars.px.all.mat.filt.1)*0.1]
dim(oars.px.all.mat.filt.1) # 904 proteins, 49 samples
...
# log transform with pseudo
oars.host.mpx.pca = log2(oars.px.all.mat.filt.1 + min(oars.px.all.mat.filt.1[oars.px.all.mat.filt.1!=0])/2)
# PCA
oars.host.mpx.pca = prcomp((oars.host.mpx.pca), scale=T)
oars.host.mpx.pca.df = oars.host.mpx.pca$x[,c(1,2)] %>% data.frame() 
oars.host.mpx.pca.df$standard.name = rownames(oars.px.all.mat.filt.1)
oars.host.mpx.pca.df = merge(oars.host.mpx.pca.df,
                                 metadata.oars.stool, by="standard.name")
oars.host.mpx.pca.var <- (oars.host.mpx.pca$sdev^2 / sum(oars.host.mpx.pca$sdev^2)) * 100

rownames(oars.host.mpx.pca.df) = oars.host.mpx.pca.df$standard.name

if(sum(colnames(oars.host.mpx.pca.df) %in% "plate")==0){
  oars.host.mpx.pca.df = merge(oars.host.mpx.pca.df, oars.mpx.map.plates, by="standard.name")
  oars.host.mpx.pca.df$plate = as.factor(oars.host.mpx.pca.df$plate)
}

# permanova
set.seed(25)
t1 = Sys.time()
oars.host.mpx.pca.permanova = vegan::adonis2(dist(oars.host.mpx.pca$x) ~ on.rs + adj.fiber + HM + plate,
                                                 oars.host.mpx.pca.df %>% mutate(on.rs = as.factor(ifelse(oars.on.rs=="onRS", "onRS", "offRS"))),
                                                 strata = oars.host.mpx.pca.df$HM,
                                                 by="margin")
t2 = Sys.time()
t2 - t1
oars.host.mpx.pca.permanova
# very not sig

oars.host.mpx.pca.plot <- ggplot(
  data=oars.host.mpx.pca.df %>% group_by(HM) %>% arrange(stool_date_rec_v2), 
  aes(x=PC1, y=PC2))+
  geom_path(aes(group=HM), color="black", linetype=2, alpha=0.5, linewidth=0.3) + 
  ...
  scale_fill_manual(values=rs.colors, na.value = "lightgrey")+
   annotate(geom="text",
            x=15,
            y=15,
            label = paste(paste("Treatment\nR²: ", round(data.frame(oars.host.mpx.pca.permanova)[1,3], 3)*100, "%",
                                "\n 𝘱: ", round(data.frame(oars.host.mpx.pca.permanova)[1,5], 3), sep="")),
            size=3.3)+
  facet_wrap(~"PCA: Host Proteins")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x=paste("PC1: ", round((oars.host.mpx.pca.var)[1], digits=2), "%", sep=""), 
       y=paste("PC2: ", round((oars.host.mpx.pca.var)[2], digits=2), "%", sep=""))
oars.host.mpx.pca.plot


# :: Maaslin2 ----------------------------------------------------

oars.px.all.mat.1[rownames(oars.px.all.mat.1) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M")& compliant==TRUE)$standard.name,] %>% nrow()
# n = 32 treatment, compliant

rownames(oars.host.mpx.pca.df) = oars.host.mpx.pca.df$standard.name

oars.host.mpx.maaslin.036 = Maaslin2::Maaslin2(input_data = (oars.px.all.mat.1),
                                              input_metadata = subset(oars.host.mpx.pca.df, timing %in% c("0M", "3M", "6M") & compliant==TRUE),
                                              output = "~/Downloads",
                                              fixed_effects = c("oars.days"),  # Example fixed effects
                                              random_effects = c("HM", "plate"),       # Example random effects
                                              normalization = "NONE",                       # Total Sum Scaling normalization
...
oars.host.mpx.maaslin.036 = oars.host.mpx.maaslin.036$results %>% data.frame() %>% arrange(pval)

# recalculate padj minus diagnosis fixed effect (if used)
oars.host.mpx.maaslin.036 = subset(oars.host.mpx.maaslin.036, metadata == "oars.days") %>%
  mutate(padj = p.adjust(pval, method="BH"))

# fix feature names
#oars.host.mpx.maaslin.036$feature = oars.mpx.cogprotein.names$good[match(oars.mpx.cog.maaslin.036$feature, oars.mpx.cogprotein.names$feature)]

oars.host.mpx.maaslin.036.volcano = ggplot(oars.host.mpx.maaslin.036,
                                          aes(x=coef, y=(padj)))+
  geom_point(shape=21, aes(fill=coef))+
  geom_hline(yintercept=(0.2), linetype=2, alpha=0.5)+
  geom_vline(xintercept=c(-1,1), linetype=2, alpha=0.5)+
  ...
  scale_y_continuous(transform=neg_log10_trans,
                     breaks=c(0.05,0.1, 0.20, 0.5, 1))+
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.50, gsub("\\..*", "", feature), NA)), 
    size=2.5, max.overlaps=100)+  facet_wrap(~"Treatment: Host Proteins")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
       y="FDR")
oars.host.mpx.maaslin.036.volcano

oars.px.all.mat.1[rownames(oars.px.all.mat.1) %in% subset(metadata.oars.stool, timing %in% c("6M", "9M", "12M")& compliant==TRUE)$standard.name,] %>% nrow()
# n = 26 washout, compliant

oars.host.mpx.maaslin.6912 = Maaslin2::Maaslin2(input_data = oars.px.all.mat.1,
                                               input_metadata = subset(oars.host.mpx.pca.df, timing %in% c("6M", "9M", "12M")& compliant==TRUE),
                                               output = "~/Downloads",
                                               fixed_effects = c("oars.days"),  # Example fixed effects
                                               random_effects = c("HM", "plate"),       # Example random effects
                                               normalization = "NONE",                       # Total Sum Scaling normalization
...
cor.test(oars.px.all.mat.1$ADH1G,
         oars.px.all.mat.1$S10A8)

oars.host.mpx.maaslin.6912 = oars.host.mpx.maaslin.6912$results %>% data.frame() %>% arrange(pval)

# recalculate padj minus diagnosis fixed effect (if used)
oars.host.mpx.maaslin.6912 = subset(oars.host.mpx.maaslin.6912, metadata == "oars.days") %>%
  mutate(padj = p.adjust(pval, method="BH"))

oars.host.mpx.maaslin.6912.volcano = ggplot(oars.host.mpx.maaslin.6912,
                                           aes(x=coef, y=(padj)))+
  geom_point(shape=21, aes(fill=coef))+
  geom_hline(yintercept=(0.2), linetype=2, alpha=0.5)+
  geom_vline(xintercept=c(-1,1), linetype=2, alpha=0.5)+
  ...
  facet_wrap(~"Washout: Host Proteins")+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text=element_text(size=10))+
  labs(x="Adjusted Coefficient",
       y="FDR")
oars.host.mpx.maaslin.6912.volcano
...
oars.host.mpx.prep = delta.omic.prepare(oars.px.all.mat.1, # norm intensity
                                        normalize=F, # don't normalize
                                        min.abun = 1, # detected at all
                                        prev=0.1) # 10% prev per group

## LMER
oars.host.mpx.lmer = delta.omic.lmer(
  split = FALSE,
  data = oars.host.mpx.prep,
  mpx = T
)

oars.host.mpx.lmer %>% arrange(pval)

subset(oars.host.mpx.lmer, taxa == "TFF3")
subset(oars.host.mpx.lmer, taxa == "S10A8")
subset(oars.host.mpx.lmer, taxa == "S10A9")

# volcano plot
oars.host.mpx.lmer.volcano = ggplot(oars.host.mpx.lmer,
                                    aes(x=estimate, y=padj))+
  geom_point(shape=21, aes(fill=estimate), size=2)+
  geom_hline(yintercept=(0.20), linetype=2, alpha=0.5)+
  geom_vline(xintercept=c(-1,1), linetype=2, alpha=0.5)+
  scale_y_continuous(transform=neg_log10_trans,
                     breaks=c(0.05,0.1, 0.20, 0.5, 1))+
  ggnetwork::geom_nodetext_repel(
    aes(label=ifelse(padj < 0.50, gsub("\\..*", "", taxa), NA)), 
    size=2.5, max.overlaps=10)+
  scale_fill_gradient2(low="blue", high="red")+
  facet_wrap(~"Interaction: Host Proteins")+
  theme_classic()+theme(legend.position="none",
                        strip.text = element_text(size=10))+
  labs(x="Interaction Coefficient", y="FDR")
oars.host.mpx.lmer.volcano

# only 1 protein with FDR < 0.20 = ADH1G (alcohol dehydrogenase, increased in strong responders)
...
oars.px$Protein.Names[grepl("S10A9", oars.px$Protein.Names)]
which(oars.px$Protein.Names == "S10A8_HUMAN") # 936
which(oars.px$Protein.Names == "S10A9_HUMAN") # 983

oars.px.clean = oars.px
colnames(oars.px.clean) = gsub("E..searches.Peter_rawstool_human.dia.", "", colnames(oars.px.clean))
colnames(oars.px.clean) = gsub("\\.raw.dia", "", colnames(oars.px.clean))
colnames(oars.px.clean) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_StintziLab_", "", colnames(oars.px.clean))
colnames(oars.px.clean) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_Stintzi_", "", colnames(oars.px.clean))
oars.px.clean = oars.px.clean[,!grepl("QC", colnames(oars.px.clean))]
colnames(oars.px.clean) = gsub("\\_", "-", colnames((oars.px.clean)))

oars.px.s100a89 = oars.px.clean[grepl((paste(c("S10A8", "S10A9"), collapse="|")), oars.px.clean$Protein.Names),-c(1:6)]

# check
plot(as.numeric(oars.px.s100a89[1,]),
     as.numeric(oars.px.s100a89[2,]))
cor.test(as.numeric(oars.px.s100a89[1,]),
         as.numeric(oars.px.s100a89[2,]), method="spearman")
# correlates well; R = 0.93, p < 0.0001

oars.s100a89 = data.frame(standard.name = colnames(oars.px.s100a89),
                          s100a8 = as.numeric(oars.px.s100a89[1,]),
                          s100a9 = as.numeric(oars.px.s100a89[2,]))

# clean up sample names
oars.s100a89$standard.name = gsub("\\.1", "", oars.s100a89$standard.name)
oars.s100a89 = subset(oars.s100a89, standard.name != "HM0926-01-A")
# good

# combine A8+A9 (and take average)
oars.s100a89 = oars.s100a89 %>%
  group_by(standard.name) %>%
  mutate(s100a8 = ifelse(is.na(s100a8), 0, s100a8),
         s100a9 = ifelse(is.na(s100a9), 0, s100a9)) %>%
  mutate(s100a8 = mean(s100a8),
         s100a9 = mean(s100a9)) %>%
  distinct() %>%
  mutate(s100a89 = log10(((na.omit(s100a8) + na.omit(s100a9)))))


# now compare to calprotectin
oars.s100a89 = merge(oars.s100a89,
                     metadata.oars.stool.asv.ph, by="standard.name", all.x=F)

# how many samples lack MPX?
sum(oars.s100a89$standard.name %in% metadata.oars.stool.asv.ph$standard.name)
# 47

range(oars.s100a89$s100a8)
range(oars.s100a89$s100a9)

plot(log10(oars.s100a89$fcal),
     oars.s100a89$s100a89)
cor.test(log10(oars.s100a89$fcal),
         oars.s100a89$s100a89, method="spearman")
# R = 0.71, p < 0.0001


# check strong vs weak fermenters

subset(oars.s100a89, HM == "HM0819")
subset(oars.s100a89.response, HM == "HM0819")

oars.s100a89.response = doubleup(subset(oars.s100a89, HM!="HM0618"), delta=T)

oars.s100a89.response$RS_Name = factor(oars.s100a89.response$RS_Name, levels=rs.names)
oars.s100a89.response$hm_phase = paste(oars.s100a89.response$HM, oars.s100a89.response$phase, sep="_")
oars.s100a89.response = subset(oars.s100a89.response, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2"))

oars.s100a89.response$response = ifelse(oars.s100a89.response$delta.ph > -1.27, "low", "high")

nrow(metadata.oars.stool.double) # n = 42 samples
nrow(oars.s100a89.response) # n = 38 samples
# lost HM0618

# note: keep only paired datapoints 
lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM), 
               subset(oars.s100a89.response, response == "low")) %>% summary() %>% coef()
lmerTest::lmer(scale(log10(fcal)) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM), 
               subset(oars.s100a89.response, response == "high")) %>% summary() %>% coef()
# note: keep only paired datapoints 
stats.s100.ph.low = lmerTest::lmer(log2(10^(s100a89)) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM) + (1|plate), 
                                   subset(oars.s100a89.response, response == "low")) %>% summary() %>% coef()
stats.s100.ph.high = lmerTest::lmer(log2(10^(s100a89)) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM)+ (1|plate), 
                                    subset(oars.s100a89.response, response == "high")) %>% summary() %>% coef()

subset(oars.s100a89.response, RS_Name == "Versafibe1490")

# make data.frame
stats.s100.ph = data.frame(Response = c("Strong Response", "Weak Response"),
                           pval = c(stats.s100.ph.high[2,5], stats.s100.ph.low[2,5]))

oars.s100a89.response.plot = ggplot(oars.s100a89.response %>%
                                      mutate(reltiming = factor(ifelse(reltiming == "pre", "Pre", "Post"), levels=c("Pre", "Post"))) %>%
                                      mutate(Response = ifelse(response == "low", "Weak Response", "Strong Response")),
                                    aes(x=reltiming, y=s100a89))+
  #scale_y_log10()+
  geom_boxplot(width=0.5)+
  geom_line(aes(group=hm_phase), linetype=2, alpha=0.5, linewidth=0.5)+
  geom_point(shape=21, aes(fill=RS_Name), size=3)+
  ...
  #ggrepel::geom_label_repel(aes(label=RS_Name, fill=RS_Name),size=4)+
  scale_fill_manual(values=rs.colors, na.value = "lightgrey")+
  geom_text(data = stats.s100.ph %>%
              mutate(Response = ifelse(Response == "Weak Response", "Weak Response", "Strong Response")), 
            x=1.5, y=(max(na.omit(oars.s100a89.response$s100a89))), 
            aes(label=paste("𝘱:", round(pval, digits=3))), size=4.5)+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x="",
       y=expression(S100A89~(Log[10]~Intensity)))

oars.s100a89.response.cor.plot = ggplot(oars.s100a89.response %>%
                                          mutate(reltiming = factor(ifelse(reltiming == "pre", "Pre", "Post"), levels=c("Pre", "Post"))) %>%
                                          mutate(Response = ifelse(response == "low", "Weak Response", "Strong Response")),
                                        aes(x=s100a89, y=fcal))+
  scale_y_log10()+
  geom_line(aes(group=hm_phase), linetype=2, alpha=0.5, linewidth=0.5)+
  geom_point(shape=21, aes(fill=RS_Name), size=3)+
  geom_hline(yintercept=250, linetype=1, alpha=1, color="red")+
  ...
  theme_classic()+theme(legend.position="none",
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(y="Fecal Calprotectin (μg/g)",
       x=expression(S100A89~(Log[10]~Sum~Intensity)))


# check lactoferrin, too
oars.px.ltf = oars.px.clean[grepl((paste(c("TRFL"), collapse="|")), oars.px.clean$Protein.Names),-c(1:6)]

plot(colSums(oars.px.s100a89),
     oars.px.ltf)
cor.test(as.vector(colSums(oars.px.s100a89)),
         t(oars.px.ltf), method="spearman")
# rho = 0.84, p < 2.2e-16

# ``` plot ----------------------------------------------------------------

((oars.s100a89.response.cor.plot+
    oars.s100a89.response.plot+
    metadata.oars.fcal.double.plot+
    patchwork::plot_layout(nrow=1, widths=c(0.8,1,1)))/
  (oars.host.mpx.pca.plot|
     oars.host.mpx.maaslin.036.volcano|
     oars.host.mpx.maaslin.6912.volcano|
     oars.host.mpx.lmer.volcano)) %>%
  ggsave(filename="./oars_plots/2026_02_12_fcal_s100_plots.pdf",
         width=14, height=7, device = cairo_pdf)

# PCA on proteins detected in >10% of samples, log2 + pseudo
# Maaslin2 using prevalence >10%, HM and plate-batch as random effects
# interaction on 10% prevalence (per group), HM and plate-batch as random effects

# :: Alain's proteins -------------------------------------------------------------

unique(oars.px$Protein.Names[grepl("FABP", oars.px$Protein.Names)])
unique(oars.px$Genes[grepl("HBD", oars.px$Genes)])

# yes: TFF3, HBD, REG3A, CLDN3, FABP2, LTF
# no:  MUC2, A1AT, HP2 (Zonulin), MPO

# loop through and extract these proteins

oars.px.others = do.call(cbind, lapply(c("TFF3", "HBD", "REG3A", "CLDN3", "FABP2", "LTF"), function(x){
  oars.px.x = oars.px[grepl(x, oars.px$Genes),]
  oars.px.x = subset(oars.px.x, Genes != "RHBDD2")
  
  oars.px.x = oars.px.x[,-c(1:6)]
  
  colnames(oars.px.x) = gsub("E..searches.Peter_rawstool_human.dia.", "", colnames(oars.px.x))
  colnames(oars.px.x) = gsub("\\.raw.dia", "", colnames(oars.px.x))
  colnames(oars.px.x) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_StintziLab_", "", colnames(oars.px.x))
  colnames(oars.px.x) = gsub("\\d{4}_\\d{2}_\\d{2}_Sara_Stintzi_", "", colnames(oars.px.x))
  oars.px.x = oars.px.x[,!grepl("QC", colnames(oars.px.x))]
  colnames(oars.px.x) = gsub("\\_", "-", colnames((oars.px.x)))
  
  oars.px.x = data.frame(standard.name = colnames(oars.px.x),
                         protein = as.numeric(oars.px.x[1,]))
  
  # clean up sample names
  oars.px.x$standard.name = gsub("\\.1", "", oars.px.x$standard.name)
  oars.px.x = subset(oars.px.x, standard.name != "HM0926-01-A")
  # combine duplicates (by mean)
  oars.px.x = oars.px.x %>%
    group_by(standard.name) %>%
    mutate(protein = ifelse(is.na(protein), 0, protein)) %>%
    mutate(protein = mean(protein)) %>%
    distinct() %>% data.frame()
  oars.px.x.df = data.frame(protein = oars.px.x[,2])
  colnames(oars.px.x.df) = x
  rownames(oars.px.x.df) = oars.px.x[,1]
  # log2
  oars.px.x.df[,1] = log2(oars.px.x.df[,1] + min(oars.px.x.df[oars.px.x.df[,1]!=0,1])/2)
  oars.px.x.df
}))
# merge with metadata
oars.px.others$standard.name = rownames(oars.px.others)
oars.px.others = merge(oars.px.others,
      metadata.oars.stool.asv.ph, by="standard.name", all.x=F)
oars.px.others

doubleup.px = function(data = beta.trajectory.data,
                    delta = FALSE,
                    compliant = TRUE){
  ...
  # if delta, output delta
  if(delta == TRUE){
    if(sum(colnames(data.036) == "TFF3")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.tff3 = TFF3[reltiming == "post"] - TFF3[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.tff3 = ((TFF3[reltiming == "pre"])))
    }
    if(sum(colnames(data.036) == "HBD")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.hbd = HBD[reltiming == "post"] - HBD[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.hbd = ((HBD[reltiming == "pre"])))
    }  
    if(sum(colnames(data.036) == "REG3A")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.reg3a = REG3A[reltiming == "post"] - REG3A[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.reg3a = ((REG3A[reltiming == "pre"])))
    }
    if(sum(colnames(data.036) == "CLDN3")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.cldn3 = CLDN3[reltiming == "post"] - CLDN3[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.cldn3 = ((CLDN3[reltiming == "pre"])))
    }
    if(sum(colnames(data.036) == "FABP2")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.fabp2 = FABP2[reltiming == "post"] - FABP2[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.fabp2 = ((FABP2[reltiming == "pre"])))
    }
    if(sum(colnames(data.036) == "LTF")==1){
      data.036 = data.036 %>%
        dplyr::group_by(HM, phase) %>%
        dplyr::mutate(delta.ltf = LTF[reltiming == "post"] - LTF[reltiming == "pre"]) %>%
        # add baseline value
        dplyr::mutate(baseline.ltf = ((LTF[reltiming == "pre"])))
    }
    # TFF3, HBD, REG3A, CLDN3, FABP2, LTF
  }
  # double up delta.ph
  data.036 = data.036 %>%
    group_by(HM, phase) %>%
...
oars.px.others.response = doubleup.px(subset(oars.px.others, HM!="HM0618"), delta=T)

oars.px.others.response$RS_Name = factor(oars.px.others.response$RS_Name, levels=rs.names)
oars.px.others.response$hm_phase = paste(oars.px.others.response$HM, oars.s100a89.response$phase, sep="_")
oars.px.others.response = subset(oars.px.others.response, !hm_phase %in% c("HM0924_rs2", "HM0932_rs2", "HM0759_rs2"))

oars.px.others.response$response = ifelse(oars.px.others.response$delta.ph > -1.27, "low", "high")

cor.test(oars.px.others$TFF3,
         oars.px.others$REG3A)

# run stats
c("TFF3", "HBD", "REG3A", "CLDN3", "FABP2", "LTF")
oars.px.others.response.stats = do.call(rbind, lapply(c("TFF3", "HBD", "REG3A", "CLDN3", "FABP2", "LTF"), function(x){
  data.subset = oars.px.others.response[,c(x, "reltiming", "phase", "diagnosis", "adj.fiber", "response", "HM", "plate")]
  colnames(data.subset)[1] = "protein"
  stats.x.ph.low = lmerTest::lmer((protein) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM) + (1|plate), 
                                   subset(data.subset, response == "low")) %>% summary() %>% coef()
  stats.x.ph.high = lmerTest::lmer((protein) ~ reltiming + phase + diagnosis + adj.fiber + (1|HM)+ (1|plate), 
                                    subset(data.subset, response == "high")) %>% summary() %>% coef()
  data.frame(protein = x,
             Response = c("Weak Response", "Strong Response"),
             coef = c(stats.x.ph.low[2,1], stats.x.ph.high[2,1]),
             pval = c(stats.x.ph.low[2,5], stats.x.ph.high[2,5]))
}))
  
oars.px.others.response.stats

oars.px.others.response.plot = ggplot(oars.px.others.response %>%
                                      mutate(reltiming = factor(ifelse(reltiming == "pre", "Pre", "Post"), levels=c("Pre", "Post"))) %>%
                                      mutate(Response = ifelse(response == "low", "Weak Response", "Strong Response")),
                                    aes(x=reltiming, y=log10(2^(TFF3))))+
  #scale_y_log10()+
  geom_boxplot(width=0.5)+
  geom_line(aes(group=hm_phase), linetype=2, alpha=0.5, linewidth=0.5)+
  geom_point(shape=21, aes(fill=RS_Name), size=3)+
  ...
  #ggrepel::geom_label_repel(aes(label=RS_Name, fill=RS_Name),size=4)+
  scale_fill_manual(values=rs.colors, na.value = "lightgrey")+
  geom_text(data = oars.px.others.response.stats %>%
              subset(protein == "TFF3") %>%
              mutate(Response = ifelse(Response == "Weak Response", "Weak Response", "Strong Response")), 
            x=1.5, y=(max(log10(2^(na.omit(oars.px.others.response$TFF3))))),
            aes(label=paste("𝘱:", round(pval, digits=3))), size=4.5)+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        ...
                        strip.background = element_rect(
                          color="black"))+
  labs(x="",
       y=expression(TFF3~(Log[10]~Intensity)))
oars.px.others.response.plot
...
# check 3-methyladipic acid, acetylcholine, adenine

# only adenine was detected (in raw annotation file, even)

oars.mbx.annotated.mat
# Adenine | 0.87min : 135.054645m/z

oars.mbx.x = oars.mbx.annotated.mat[,grepl("Adenine", colnames(oars.mbx.annotated.mat))]
...
# Goal: per Dimitri's request, generate heatmaps for all features (group + responders)
# use p < 0.1 as sig

# plot LFC; subset to p < 0.1, highlight p < 0.05 + and FDR < 0.20 * and <0.05 ***

# Mapping files
oars.omic.heatmap.mapping.1 = list(
  Patient = c(C03 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[1],
              U09 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[2],
              U13 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[3],
              U11 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[4],
              C02 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[5],
              C06 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[6],
              C01 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[7],
              C05 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[8],
              U10 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[9],
              C04 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[10],
              C07 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[11],
              U12 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[12]),
  RS_Type = rs.colors.type,
  Coefficient = colorRampPalette(c("blue","white", "red"))(100),
  Fiber = colorRampPalette(c("blue","white", "red"))(100),
  Diagnosis = c(UC = labelcolors$cols[c(1)],
                CD = labelcolors$cols[c(7)]),
  Significance =  c(`𝑝 < 0.05` = "lightgrey",  `FDR < 0.20` = "lightblue", `FDR < 0.05` = "blue"),
  Timing = c(`0M` = "white", `3M` = "grey", `6M` = "black")
)


oars.omic.heatmap.mapping.2 = list(
  Patient = c(C03 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[1],
              U09 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[2],
              U13 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[3],
              U11 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[4],
  ...
  Diagnosis = c(UC = labelcolors$cols[c(1)],
                CD = labelcolors$cols[c(7)]),
  Significance =  c(`𝑝 < 0.05` = "lightgrey",  `FDR < 0.20` = "lightblue", `FDR < 0.05` = "blue"),
  Timing = c(`6M` = "white", `9M` = "grey", `12M` = "black")
)

# name shortener
name.shortener = function(original = original,
                          length = 40){
  # set names
  names = original
  
  # clean ASV names
  names = ifelse(grepl("_s__", names), gsub("g__", "(s) ", names), names)
  names = gsub("_s__", " ", names)
  names = gsub("g__", "(g) ",names)
  names = gsub("f__", "(f) ",names)
  names = gsub("o__", "(o) ",names)
  names = gsub("c__", "(c) ",names)
  ...
  # clean COG names
  #names = gsub("\\, .*", "", names)
  #names = gsub(" or related.*", "", names)
  # reduce to 40 char
  names = strtrim(names, width = length)
  names = ifelse(nchar(names) == length, paste(names, "...", sep=""), names)
  ...
  # now set names as dataframe
  return(data.frame(new = names))
  
}

rs.type.code = data.frame(RS_Name = rs.names,
           RS_Type = c("Potato", "Potato", "Potato",
                       "Green banana", 
                       "Corn", "Corn", 
...
subset(oars.asv.maaslin.036, pval <= 0.1)
subset(oars.asv.maaslin.6912, pval <= 0.1)
oars.asv.data.glom
subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID
metadata.oars.stool.asv = metadata.oars.stool.asv %>% 
  merge(oars.deid) %>%
  mutate(standard.name.2 = paste(code, timing))
rownames(oars.asv.data.glom) = metadata.oars.stool.asv$standard.name.2[match(rownames(oars.asv.data.glom),
                                                                             metadata.oars.stool.asv$standard.name)]

oars.asv.heatmap.data.treatment = (oars.asv.data.glom[subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                      colnames(as.data.frame(oars.asv.data.glom)) %in% 
                     subset(oars.asv.maaslin.036, pval <= 0.1)$feature]/500+
  (min(oars.asv.data.glom[oars.asv.data.glom!=0]/500)/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 

oars.asv.heatmap.data.washout = (oars.asv.data.glom[subset(metadata.oars.stool.asv, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                      colnames(as.data.frame(oars.asv.data.glom)) %in% 
                                                        subset(oars.asv.maaslin.6912, pval <= 0.1)$feature]/500+
                                     (min(oars.asv.data.glom[oars.asv.data.glom!=0]/500)/2)) %>% log10() %>%
  as.matrix() %>% 
  scale() %>%  # scaled per column
  t() 

pheatmap::pheatmap((oars.asv.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.asv.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.asv.heatmap.data.treatment)/100, max(oars.asv.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.asv.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     column_to_rownames("feature") %>%
                     dplyr::select(sig, coef) %>%
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            Timing = timing,
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   labels_row = name.shortener(rownames(oars.asv.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.asv.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "ASV | Treatment Phase",
                   border_color = "white",
                   cellwidth = 9)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_asv_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)


pheatmap::pheatmap((oars.asv.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.asv.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.asv.heatmap.data.washout)/100, max(oars.asv.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.asv.maaslin.6912, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     column_to_rownames("feature") %>%
                     dplyr::select(sig, coef) %>%
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool.asv, timing %in% c("9M", "12M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% 
                     mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            Timing = timing,
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.asv.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.asv.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "ASV | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_asv_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
subset(oars.mgx.maaslin.036, pval <= 0.1)
subset(oars.mgx.maaslin.6912, pval <= 0.1)
oars.mgx.taxa
subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID is done
rownames(oars.mgx.taxa) = metadata.oars.stool.asv$standard.name.2[match(rownames(oars.mgx.taxa),
                                                                             metadata.oars.stool.asv$standard.name)]

oars.mgx.heatmap.data.treatment = (data.frame(oars.mgx.taxa)[rownames(oars.mgx.taxa) %in% subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                      colnames(as.data.frame(oars.mgx.taxa)) %in% 
                                                        subset(oars.mgx.maaslin.036, pval <= 0.1)$feature]+
                                     (min(oars.mgx.taxa[oars.mgx.taxa!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 

oars.mgx.heatmap.data.washout = (data.frame(oars.mgx.taxa)[rownames(oars.mgx.taxa) %in% subset(metadata.oars.stool.asv, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                    colnames(as.data.frame(oars.mgx.taxa)) %in% 
                                                      subset(oars.mgx.maaslin.6912, pval <= 0.1)$feature]+
                                   (min(oars.mgx.taxa[oars.mgx.taxa!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


colnames(oars.mgx.heatmap.data.treatment) = gsub("\\.", " ", colnames(oars.mgx.heatmap.data.treatment))

pheatmap::pheatmap((oars.mgx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mgx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mgx.heatmap.data.treatment)/100, max(oars.mgx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.mgx.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)[,c("code", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.mgx.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.mgx.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Species | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_mgx_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)

colnames(oars.mgx.heatmap.data.washout) = gsub("\\.", " ", colnames(oars.mgx.heatmap.data.washout))

pheatmap::pheatmap((oars.mgx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mgx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mgx.heatmap.data.washout)/100, max(oars.mgx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.mgx.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool.asv, timing %in% c("9M", "12M", "6M") & compliant==TRUE)[,c("code", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.mgx.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.mgx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Species | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12) %>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_mgx_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
subset(oars.mpx.kegg.maaslin.036, pval <= 0.1)
subset(oars.mpx.kegg.maaslin.6912, pval <= 0.1)
oars.mpx.kegg.mat.hm = oars.mpx.kegg.mat # heatmap
subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID
metadata.oars.stool = metadata.oars.stool %>% 
  merge(oars.deid) %>%
  mutate(standard.name.2 = paste(code, timing))
rownames(oars.mpx.kegg.mat.hm) = metadata.oars.stool$standard.name.2[match(rownames(oars.mpx.kegg.mat.hm),
                                                                       metadata.oars.stool$standard.name)]
# "HM0924-STL-12" = "HM0924-STL-11", both are 3M

oars.kegg.heatmap.data.treatment = oars.mpx.kegg.mat.hm
oars.kegg.heatmap.data.treatment[is.na(oars.kegg.heatmap.data.treatment)] = 0
oars.kegg.heatmap.data.treatment =  ((oars.kegg.heatmap.data.treatment)[rownames(oars.kegg.heatmap.data.treatment) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                             colnames(as.data.frame(oars.kegg.heatmap.data.treatment)) %in% 
                                                               subset(oars.mpx.kegg.maaslin.036, pval <= 0.1)$feature]+
                                     (min(oars.kegg.heatmap.data.treatment[(oars.kegg.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

oars.kegg.heatmap.data.washout = oars.mpx.kegg.mat.hm
oars.kegg.heatmap.data.washout[is.na(oars.kegg.heatmap.data.washout)] = 0
oars.kegg.heatmap.data.washout = ((oars.kegg.heatmap.data.washout)[rownames(oars.kegg.heatmap.data.washout) %in% subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                           colnames(as.data.frame(oars.kegg.heatmap.data.washout)) %in% 
                                                             subset(oars.mpx.kegg.maaslin.6912, pval <= 0.1)$feature]+
                                   (min(oars.kegg.heatmap.data.washout[oars.kegg.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


pheatmap::pheatmap((oars.kegg.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.kegg.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.kegg.heatmap.data.treatment)/100, 
                                max(oars.kegg.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.kegg.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M")& compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.kegg.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.kegg.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 10)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_kegg_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)


pheatmap::pheatmap((oars.kegg.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.kegg.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.kegg.heatmap.data.washout)/100, max(oars.kegg.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.kegg.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M")& compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     merge(rs.type.code, all.x = T) %>%
                     merge(oars.deid) %>% mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.kegg.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.kegg.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_kegg_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)

# :: :: COG ------------------------------------------------------------------

subset(oars.mpx.cog.maaslin.036, pval <= 0.1)
subset(oars.mpx.cog.maaslin.6912, pval <= 0.1)
oars.mpx.cog.mat.hm = oars.mpx.cog.mat
subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID is done
rownames(oars.mpx.cog.mat.hm) = metadata.oars.stool$standard.name.2[match(rownames(oars.mpx.cog.mat.hm),
                                                                        metadata.oars.stool$standard.name)]


oars.cog.heatmap.data.treatment = oars.mpx.cog.mat.hm
oars.cog.heatmap.data.treatment[is.na(oars.cog.heatmap.data.treatment)] = 0
oars.cog.heatmap.data.treatment =  ((oars.cog.heatmap.data.treatment)[rownames(oars.cog.heatmap.data.treatment) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                        colnames(as.data.frame(oars.cog.heatmap.data.treatment)) %in% 
                                                                          subset(oars.mpx.cog.maaslin.036, pval <= 0.1)$feature]+
                                       (min(oars.cog.heatmap.data.treatment[(oars.cog.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

oars.cog.heatmap.data.washout = oars.mpx.cog.mat.hm
oars.cog.heatmap.data.washout[is.na(oars.cog.heatmap.data.washout)] = 0
oars.cog.heatmap.data.washout = ((oars.cog.heatmap.data.washout)[rownames(oars.cog.heatmap.data.washout) %in% subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                                   colnames(as.data.frame(oars.cog.heatmap.data.washout)) %in% 
                                                                     subset(oars.mpx.cog.maaslin.6912, pval <= 0.1)$feature]+
                                    (min(oars.cog.heatmap.data.washout[oars.cog.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


pheatmap::pheatmap((oars.cog.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cog.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cog.heatmap.data.treatment)/100, 
                                max(oars.cog.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cog.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.cog.heatmap.data.treatment))$new,
                   fontsize_row = 3,
                   fontsize_col = 7,
                   main = "COG | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_cog_tr_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=11)


pheatmap::pheatmap((oars.cog.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cog.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cog.heatmap.data.washout)/100, max(oars.cog.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cog.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.cog.heatmap.data.washout))$new,
                   fontsize_row = 3,
                   fontsize_col = 7,
                   main = "COG | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_cog_wo_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=11)

# :: :: CAZy ------------------------------------------------------------------

subset(oars.mpx.cazy.maaslin.036, pval <= 0.1)
subset(oars.mpx.cazy.maaslin.6912, pval <= 0.1)
oars.mpx.cazy.mat.hm = oars.mpx.cazy.mat
subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID is done
rownames(oars.mpx.cazy.mat.hm) = metadata.oars.stool$standard.name.2[match(rownames(oars.mpx.cazy.mat.hm),
                                                                              metadata.oars.stool$standard.name)]


oars.cazy.heatmap.data.treatment = oars.mpx.cazy.mat.hm
oars.cazy.heatmap.data.treatment[is.na(oars.cazy.heatmap.data.treatment)] = 0
oars.cazy.heatmap.data.treatment =  ((oars.cazy.heatmap.data.treatment)[rownames(oars.cazy.heatmap.data.treatment) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.cazy.heatmap.data.treatment)) %in% 
                                                                        subset(oars.mpx.cazy.maaslin.036, pval <= 0.1)$feature]+
                                      (min(oars.cazy.heatmap.data.treatment[(oars.cazy.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

oars.cazy.heatmap.data.washout = oars.mpx.cazy.mat.hm
oars.cazy.heatmap.data.washout[is.na(oars.cazy.heatmap.data.washout)] = 0
oars.cazy.heatmap.data.washout = ((oars.cazy.heatmap.data.washout)[rownames(oars.cazy.heatmap.data.washout) %in% subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                                 colnames(as.data.frame(oars.cazy.heatmap.data.washout)) %in% 
                                                                   subset(oars.mpx.cazy.maaslin.6912, pval <= 0.1)$feature]+
                                   (min(oars.cazy.heatmap.data.washout[oars.cazy.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


pheatmap::pheatmap((oars.cazy.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cazy.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cazy.heatmap.data.treatment)/100, 
                                max(oars.cazy.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cazy.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M")& compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.cazy.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.cazy.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_cazy_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)


pheatmap::pheatmap((oars.cazy.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cazy.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cazy.heatmap.data.washout)/100, max(oars.cazy.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cazy.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.cazy.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.cazy.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 11)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_cazy_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)

# :: :: MBX ------------------------------------------------------------------

subset(oars.mbx.maaslin.036, pval <= 0.1)
subset(oars.mbx.maaslin.6912, pval <= 0.1)
oars.mbx.raw.mat.filt.1.hm = oars.mbx.raw.mat.filt.1
subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID is done
rownames(oars.mbx.raw.mat.filt.1.hm) = metadata.oars.stool$standard.name.2[match(rownames(oars.mbx.raw.mat.filt.1.hm),
                                                                               metadata.oars.stool$standard.name)]


oars.mbx.heatmap.data.treatment = oars.mbx.raw.mat.filt.1.hm
oars.mbx.heatmap.data.treatment[is.na(oars.mbx.heatmap.data.treatment)] = 0
oars.mbx.heatmap.data.treatment =  ((oars.mbx.heatmap.data.treatment)[rownames(oars.mbx.heatmap.data.treatment) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.mbx.heatmap.data.treatment)) %in% 
                                                                        subset(oars.mbx.maaslin.036, pval <= 0.1)$feature]+
                                      (min(oars.mbx.heatmap.data.treatment[(oars.mbx.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

oars.mbx.heatmap.data.washout = oars.mbx.raw.mat.filt.1.hm
oars.mbx.heatmap.data.washout[is.na(oars.mbx.heatmap.data.washout)] = 0
oars.mbx.heatmap.data.washout = ((oars.mbx.heatmap.data.washout)[rownames(oars.mbx.heatmap.data.washout) %in% subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                                 colnames(as.data.frame(oars.mbx.heatmap.data.washout)) %in% 
                                                                   subset(oars.mbx.maaslin.6912, pval <= 0.1)$feature]+
                                   (min(oars.mbx.heatmap.data.washout[oars.mbx.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


pheatmap::pheatmap((oars.mbx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mbx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mbx.heatmap.data.treatment)/100, 
                                max(oars.mbx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.mbx.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.mbx.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.mbx.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 10)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_mbx_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)


pheatmap::pheatmap((oars.mbx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mbx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mbx.heatmap.data.washout)/100, max(oars.mbx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.mbx.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M")& compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.mbx.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.mbx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_mbx_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)

# :: :: Host MPX ------------------------------------------------------------------

subset(oars.host.mpx.maaslin.036, pval <= 0.1)
subset(oars.host.mpx.maaslin.6912, pval <= 0.1)
oars.px.all.mat.1.hm = oars.px.all.mat.1
subset(oars.host.mpx.pca.df, timing %in% c("0M", "3M", "6M") & compliant==TRUE)
# DEID
oars.host.mpx.pca.df = oars.host.mpx.pca.df %>% 
  merge(oars.deid) %>%
  mutate(standard.name.2 = paste(code, timing))
rownames(oars.px.all.mat.1.hm) = oars.host.mpx.pca.df$standard.name.2[match(rownames(oars.px.all.mat.1.hm),
                                                                          oars.host.mpx.pca.df$standard.name)]


oars.host.mpx.heatmap.data.treatment = oars.px.all.mat.1.hm
oars.host.mpx.heatmap.data.treatment[is.na(oars.host.mpx.heatmap.data.treatment)] = 0
oars.host.mpx.heatmap.data.treatment =  ((oars.host.mpx.heatmap.data.treatment)[rownames(oars.host.mpx.heatmap.data.treatment) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.host.mpx.heatmap.data.treatment)) %in% 
                                                                        subset(oars.host.mpx.maaslin.036, pval <= 0.1)$feature]+
                                      (min(oars.host.mpx.heatmap.data.treatment[(oars.host.mpx.heatmap.data.treatment)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

oars.host.mpx.heatmap.data.washout = oars.px.all.mat.1.hm
oars.host.mpx.heatmap.data.washout[is.na(oars.host.mpx.heatmap.data.washout)] = 0
oars.host.mpx.heatmap.data.washout = ((oars.host.mpx.heatmap.data.washout)[rownames(oars.host.mpx.heatmap.data.washout) %in% subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M") & compliant==TRUE)$standard.name.2,
                                                                 colnames(as.data.frame(oars.host.mpx.heatmap.data.washout)) %in% 
                                                                   subset(oars.host.mpx.maaslin.6912, pval <= 0.1)$feature]+
                                   (min(oars.host.mpx.heatmap.data.washout[oars.host.mpx.heatmap.data.washout!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 


pheatmap::pheatmap((oars.host.mpx.heatmap.data.treatment),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.host.mpx.heatmap.data.treatment), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.host.mpx.heatmap.data.treatment)/100, 
                                max(oars.host.mpx.heatmap.data.treatment), length.out=floor(100/2))),
                   annotation_row = subset(oars.host.mpx.maaslin.036, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.host.mpx.heatmap.data.treatment))$new,
                   fontsize_row = 10 - nrow(oars.host.mpx.heatmap.data.treatment) / 15,
                   fontsize_col = 7,
                   main = "Host Protein | Treatment Phase",
                   annotation_colors = oars.omic.heatmap.mapping.1,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_host_mpx_tr_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)


pheatmap::pheatmap((oars.host.mpx.heatmap.data.washout),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.host.mpx.heatmap.data.washout), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.host.mpx.heatmap.data.washout)/100, max(oars.host.mpx.heatmap.data.washout), length.out=floor(100/2))),
                   annotation_row = subset(oars.host.mpx.maaslin.6912, pval <= 0.1) %>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     remove_rownames %>% 
                     column_to_rownames("feature") %>%
                   ...
                   annotation_col = subset(metadata.oars.stool, timing %in% c("9M", "12M", "6M")& compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "standard.name")] %>%
                     ...
                     mutate(Diagnosis = diagnosis,
                            ...
                            Fiber = adj.fiber,
                            Patient = code) %>% dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient),
                   labels_row = name.shortener(rownames(oars.host.mpx.heatmap.data.washout))$new,
                   fontsize_row = 10 - nrow(oars.host.mpx.heatmap.data.washout) / 15,
                   fontsize_col = 7,
                   main = "Host Protein | Washout Phase",
                   annotation_colors = oars.omic.heatmap.mapping.2,
                   border_color = "white",
                   cellwidth = 12)%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_group_host_mpx_wo_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=11)
...
oars.omic.heatmap.mapping.resp = list(
  Patient = c(C03 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[1],
              U09 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[2],
              U13 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[3],
              U11 = RColorBrewer::brewer.pal(n = 12, name = "Set3")[4],
  ...
  Significance =  c(`𝑝 < 0.05` = "lightgrey",  `FDR < 0.20` = "lightblue", `FDR < 0.05` = "blue"),
  Timing = c(`0M` = "white", `3M` = "grey", `6M` = "black"),
  Response = c(Strong = labelcolors$cols[c(1)],
                Weak = labelcolors$cols[c(7)])
)


# :: :: ASV ---------------------------------------------------------------

oars.asv.data.glom.lmer

oars.asv.heatmap.data.responder = (oars.asv.data.glom[subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                      colnames(as.data.frame(oars.asv.data.glom)) %in% 
                                                        subset(oars.asv.data.glom.lmer, pval <= 0.1)$taxa]/500+
                                     (min(oars.asv.data.glom[oars.asv.data.glom!=0]/500)/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 

pheatmap::pheatmap((oars.asv.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.asv.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.asv.heatmap.data.responder)/100, max(oars.asv.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.asv.data.glom.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     ...
                     mutate(Coefficient = coef,
                            Significance = sig) %>% dplyr::select(Coefficient, Significance),
                   annotation_col = subset(metadata.oars.stool.double, 
                                           timing %in% c("0M", "3M", "6M") & 
                                             compliant==TRUE)[,c("HM", "timing", "diagnosis", "RS_Name", "adj.fiber", "response", "standard.name", "phase")] %>% distinct() %>%
                     merge(rs.type.code, all.x=T) %>%
                     merge(oars.deid) %>%
                     mutate(RS_Type = factor(RS_Type, levels=names(rs.colors.type))) %>%
                     mutate(Diagnosis = diagnosis,
                            ...
                            Response = ifelse(
                              timing == "0M", NA, ifelse(
                              response == "low", "Weak", "Strong")),
                            Patient = code) %>% 
                     subset(phase != "rs2" | timing != "3M") %>%
                     remove_rownames() %>%
                     mutate(standard.name = paste(code, timing)) %>%
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.asv.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.asv.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "ASV | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white") %>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_asv_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
oars.mgx.heatmap.data.responder = (data.frame(oars.mgx.taxa)[rownames(oars.mgx.taxa) %in% subset(metadata.oars.stool.asv, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                      colnames(as.data.frame(oars.mgx.taxa)) %in% 
                                                        subset(oars.mgx.lmer, pval <= 0.1)$taxa]+
                                     (min(oars.mgx.taxa[oars.mgx.taxa!=0])/2)) %>% log10() %>%
  as.matrix() %>% scale() %>% t() 

colnames(oars.mgx.heatmap.data.responder) = gsub("\\.", " ", colnames(oars.mgx.heatmap.data.responder))

pheatmap::pheatmap((oars.mgx.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mgx.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mgx.heatmap.data.responder)/100, max(oars.mgx.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.mgx.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.mgx.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.mgx.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "Species | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_mgx_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)

# :: :: KEGG ------------------------------------------------------------------

oars.kegg.heatmap.data.responder = oars.mpx.kegg.mat.hm
oars.kegg.heatmap.data.responder[is.na(oars.kegg.heatmap.data.responder)] = 0
oars.kegg.heatmap.data.responder =  ((oars.kegg.heatmap.data.responder)[rownames(oars.kegg.heatmap.data.responder) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                        colnames(as.data.frame(oars.kegg.heatmap.data.responder)) %in% 
                                                                          subset(oars.mpx.kegg.lmer, pval <= 0.1)$taxa]+
                                       (min(oars.kegg.heatmap.data.responder[(oars.kegg.heatmap.data.responder)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

pheatmap::pheatmap((oars.kegg.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.kegg.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.kegg.heatmap.data.responder)/100, max(oars.kegg.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.kegg.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.kegg.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.kegg.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "Pathway | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_kegg_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)

# :: :: COG ------------------------------------------------------------------

oars.cog.heatmap.data.responder = oars.mpx.cog.mat.hm
oars.cog.heatmap.data.responder[is.na(oars.cog.heatmap.data.responder)] = 0
oars.cog.heatmap.data.responder =  ((oars.cog.heatmap.data.responder)[rownames(oars.cog.heatmap.data.responder) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                        colnames(as.data.frame(oars.cog.heatmap.data.responder)) %in% 
                                                                          subset(oars.mpx.cog.lmer, pval <= 0.1)$taxa]+
                                       (min(oars.cog.heatmap.data.responder[(oars.cog.heatmap.data.responder)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

pheatmap::pheatmap((oars.cog.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cog.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cog.heatmap.data.responder)/100, max(oars.cog.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cog.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.cog.heatmap.data.responder))$new,
                   fontsize_row = 3,
                   fontsize_col = 7,
                   main = "COG | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_cog_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
oars.cazy.heatmap.data.responder = oars.mpx.cazy.mat.hm
oars.cazy.heatmap.data.responder[is.na(oars.cazy.heatmap.data.responder)] = 0
oars.cazy.heatmap.data.responder =  ((oars.cazy.heatmap.data.responder)[rownames(oars.cazy.heatmap.data.responder) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.cazy.heatmap.data.responder)) %in% 
                                                                        subset(oars.mpx.cazy.lmer, pval <= 0.1)$taxa]+
                                      (min(oars.cazy.heatmap.data.responder[(oars.cazy.heatmap.data.responder)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

pheatmap::pheatmap((oars.cazy.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.cazy.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.cazy.heatmap.data.responder)/100, max(oars.cazy.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.mpx.cazy.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.cazy.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.cazy.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "CAZyme | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_cazy_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)
...
oars.mbx.heatmap.data.responder = oars.mbx.raw.mat.filt.1.hm
oars.mbx.heatmap.data.responder[is.na(oars.mbx.heatmap.data.responder)] = 0
oars.mbx.heatmap.data.responder =  ((oars.mbx.heatmap.data.responder)[rownames(oars.mbx.heatmap.data.responder) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.mbx.heatmap.data.responder)) %in% 
                                                                        subset(oars.mbx.lmer, pval <= 0.1)$taxa]+
                                      (min(oars.mbx.heatmap.data.responder[(oars.mbx.heatmap.data.responder)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

pheatmap::pheatmap((oars.mbx.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.mbx.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.mbx.heatmap.data.responder)/100, max(oars.mbx.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.mbx.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.mbx.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.mbx.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "Metabolite | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_mbx_heatmap.pdf",
         device = cairo_pdf,
         width=10, height=12)

# :: :: Host MPX ------------------------------------------------------------------

oars.host.mpx.heatmap.data.responder = oars.px.all.mat.1.hm
oars.host.mpx.heatmap.data.responder[is.na(oars.host.mpx.heatmap.data.responder)] = 0
oars.host.mpx.heatmap.data.responder =  ((oars.host.mpx.heatmap.data.responder)[rownames(oars.host.mpx.heatmap.data.responder) %in% subset(metadata.oars.stool, timing %in% c("0M", "3M", "6M") & compliant==TRUE)$standard.name.2,
                                                                      colnames(as.data.frame(oars.host.mpx.heatmap.data.responder)) %in% 
                                                                        subset(oars.host.mpx.lmer, pval <= 0.1)$taxa]+
                                      (min(oars.host.mpx.heatmap.data.responder[(oars.host.mpx.heatmap.data.responder)!=0])/2)) %>% log10() %>%
  scale() %>% t() 

pheatmap::pheatmap((oars.host.mpx.heatmap.data.responder),
                   color=colorRampPalette(c("blue","white", "red"))(100),
                   clustering_distance_rows = "correlation",
                   clustering_distance_cols = "correlation",
                   breaks=c(seq(min(oars.host.mpx.heatmap.data.responder), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.host.mpx.heatmap.data.responder)/100, max(oars.host.mpx.heatmap.data.responder), length.out=floor(100/2))),
                   annotation_row = subset(oars.host.mpx.lmer, pval <= 0.1)%>%
                     mutate(sig = ifelse(padj < 0.05, "FDR < 0.05", ifelse(padj < 0.20, "FDR < 0.20", ifelse(pval < 0.05, "𝑝 < 0.05", ""))))%>%
                     mutate(sig = factor(sig, levels=c("𝑝 < 0.05", "FDR < 0.20", "FDR < 0.05"))) %>%
                     mutate(coef = estimate)%>%
                     remove_rownames %>% 
                   ...
                   annotation_col = subset(metadata.oars.stool.double, 
                     ...
                     column_to_rownames("standard.name") %>%
                     dplyr::select(Diagnosis, Timing, Fiber, RS_Type, Patient, Response),
                   labels_row = name.shortener(rownames(oars.host.mpx.heatmap.data.responder))$new,
                   fontsize_row = 10 - nrow(oars.host.mpx.heatmap.data.responder) / 15,
                   fontsize_col = 7,
                   main = "Host Protein | Responders",
                   annotation_colors = oars.omic.heatmap.mapping.resp,
                   border_color = "white")%>%
  ggsave(filename="./oars_plots_deidentified/heatmaps/oars_response_host_mpx_heatmap_DI.pdf",
         device = cairo_pdf,
         width=10, height=12)

# :: Save tables ----------------------------------------------------------

# Goal: per Dimitri's request, generate tables for all features (group + responders)
# use p < 0.1 as sig

oars.asv.maaslin.036
oars.asv.maaslin.6912

oars.mgx.maaslin.036
oars.mgx.maaslin.6912

oars.kegg.maaslin.036
oars.kegg.maaslin.6912

oars.cog.maaslin.036
oars.cog.maaslin.6912

oars.cazy.maaslin.036
oars.cazy.maaslin.6912

oars.mbx.maaslin.036
oars.mbx.maaslin.6912
...
openxlsx::write.xlsx(
  list(
    comments = data.frame(notes = 
                            c("tabs named `group` used MaAsLin2, fitting log-transformed features with >=10% prevalence, participant identity as a random-effect, and Benjamini-Hochberg adjustment",
                            "tabs named `response` used lmer, testing the interaction between strong & weak fermenters, using the same parameters as MaAsLin2, plus phase (RS1 or RS2) as a covariate",
                            "tr = treatment phase (0M baseline through 6M)", "wo = washout phase (6M end of RS through 12M)", 
                            "asv = ASVs agglomerated to the lowest common annotated ancestor (16S data)",
                            "mgx = metagenomic species",
                            "kegg = KEGG pathway (metaproteomics)",
                            "cog = COG-annotated proteins (metaproteomics)",
                            "cazy = CAZyme (metaproteomics)",
                            "mbx = metabolites",
                            "host mpx = proteomics annotated with human proteins",
                            "correlation tabs were calculated using Spearman correlation coefficients and Benjamini-Hochberg adjustment")),
    `st 5 asv_tr` = oars.asv.maaslin.036 %>% mutate(data.type = "ASV", timing="treatment"),
    `st 6 asv_wo` = oars.asv.maaslin.6912 %>% mutate(data.type = "ASV", timing="washout"),
    `st 7 mgx_tr` = oars.mgx.maaslin.036 %>% mutate(data.type = "Species", timing="treatment"),
    `st 8 mgx_wo` = oars.mgx.maaslin.6912 %>% mutate(data.type = "Species", timing="washout"),
    `st 9 kegg_tr` = oars.mpx.kegg.maaslin.036 %>% mutate(data.type = "Pathway", timing="treatment"),
    `st 10 kegg_wo` = oars.mpx.kegg.maaslin.6912 %>% mutate(data.type = "Pathway", timing="washout"),
    `st 11 cog_tr` = oars.mpx.cog.maaslin.036 %>% mutate(data.type = "COG", timing="treatment"),
    `st 12 cog_wo` = oars.mpx.cog.maaslin.6912 %>% mutate(data.type = "COG", timing="washout"),
    `st 13 cazy_tr` = oars.mpx.cazy.maaslin.036 %>% mutate(data.type = "CAZyme", timing="treatment"),
    `st 14 cazy_wo` = oars.mpx.cazy.maaslin.6912 %>% mutate(data.type = "CAZyme", timing="washout"),
    `st 15 mbx_tr` = oars.mbx.maaslin.036 %>% mutate(data.type = "Metabolite", timing="treatment"),
    `st 16 mbx_wo` = oars.mbx.maaslin.6912 %>% mutate(data.type = "Metabolite", timing="washout"),
    `st 17 host_mpx_tr` = oars.host.mpx.maaslin.036 %>% mutate(data.type = "Host protein", timing="treatment"),
    `st 18 host_mpx_wo` = oars.host.mpx.maaslin.6912 %>% mutate(data.type = "Host protein", timing="washout"),
    `st 19 resp_asv` = oars.asv.data.glom.lmer %>% mutate(data.type = "ASV"),
    `st 20 resp_mgx` = oars.mgx.lmer %>% mutate(data.type = "Species"),
    `st 21 resp_kegg` = oars.mpx.kegg.lmer %>% mutate(data.type = "Pathway"),
    `st 22 resp_cog` = oars.mpx.cog.lmer %>% mutate(data.type = "COG"),
    `st 23 resp_cazy` = oars.mpx.cazy.lmer %>% mutate(data.type = "CAZyme"),
    `st 24 resp_mbx` = oars.mbx.lmer %>% mutate(data.type = "Metabolite"),
    `st 25 resp_host_mpx` = oars.host.mpx.lmer %>% mutate(data.type = "Host protein"),
    `st 26 fcal_cor` = oars.fcal.cor.data
    #`st 27 all_cor_sig` = subset(oars.all.cor.data, padj < 0.20)
  ),
  
  "oars_paper/tables/oars_heatmap_source.xlsx")
...
metadata.oars.stool.plot %>%
  ggsave(filename="./oars_plots_deidentified/2026_02_05_oars_1_stools_DI.pdf",
         width=5, height=3.3)
...
(metadata.oars.but.i.plot + 
  ...
  ggsave(filename="./oars_plots/2026_02_05_oars_2_group_level.pdf",
...
((((oars.asv.pcoa.plot+
  ...
  oars.pca.permanova.plot)+patchwork::plot_layout(widths=c(2.2,1)))%>%
  ggsave(filename="./oars_plots/2026_02_05_oars_2_omics_pca.pdf",
         width=9, height=8)

# Figure 7B: Omics LFC 
rownames(oars.group.mgx.mpx.mbx.lfc.map.map) = gsub(" \\|.*", "", rownames(oars.group.mgx.mpx.mbx.lfc.map.map))
oars.group.mgx.mpx.mbx.lfc.map.map$Datatype = ifelse(oars.group.mgx.mpx.mbx.lfc.map.map$Datatype == "CAZy", "CAZyme", 
                                                     oars.group.mgx.mpx.mbx.lfc.map.map$Datatype)
oars.omics.heatmap.mapping = merge(oars.omics.heatmap.mapping, data.frame(
  RS = rs.names,
  rs.type = c("Potato", "Potato", "Potato", "Green banana", "Corn", "Corn", "Cross-linked", "Cross-linked", "Cross-linked potato")),
  by="RS") %>%
  dplyr::select(-RS) %>%
  mutate(`RS Type`= rs.type) %>%
  dplyr::select(-rs.type) 
oars.omics.heatmap.mapping = oars.omics.heatmap.mapping %>%
  merge(oars.deid %>% mutate(Patient = HM))
rownames(oars.omics.heatmap.mapping) = paste(oars.omics.heatmap.mapping$code, 
                                             oars.omics.heatmap.mapping$Timing, sep= " ")

pheatmap::pheatmap(reshape2::acast(oars.group.mgx.mpx.mbx.lfc %>% 
                                     merge(oars.deid) %>%
                                     mutate(sample2 = paste(code, timing)) %>%
                   ...
                   annotation_row = oars.group.mgx.mpx.mbx.lfc.map.map %>%
                     mutate(Datatype = factor(Datatype, levels=names(omics.colors))),
                   annotation_col = oars.omics.heatmap.mapping[,!colnames(oars.omics.heatmap.mapping) %in%c("Patient", "code", "HM")],
                   annotation_colors = list(Coefficient = colorRampPalette(c("blue","white", "red"))(100),
                                            Datatype = omics.colors,
                                            Timing = c("3M" = "white", "6M" = "lightgrey"),
                                            `RS Type`= rs.colors.type),
                   breaks=c(seq(min(oars.group.mgx.mpx.mbx.lfc$slfc), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(oars.group.mgx.mpx.mbx.lfc$slfc)/100, max(oars.group.mgx.mpx.mbx.lfc$slfc), 
                                length.out=floor(100/2))),
                   border="white") %>%
  ggsave(filename="./oars_plots_deidentified/2026_02_05_oars_2_omics_heatmap_DI.pdf",
...
stats.tree.features.plots.top = oars.double.response.tree.lm.plot+
  patchwork::plot_spacer()+
  patchwork::free(oars.double.response.tree.lm.sd.plot,type="label")+
  patchwork::plot_layout(widths=c(2.75,0.3,1))


cowplot::plot_grid(stats.tree.features.plots.top, stats.tree.features.plots,
                   nrow=2, rel_heights=c(3,1))%>%
  ggsave(filename="./oars_plots/2026_02_05_oars_2_starch_fermentation_stream.pdf",
...
((oars.ph.rs.boxplot+
  ...
  patchwork::plot_layout(widths=c(2,1,1,1.2))) %>%
  ggsave(filename="./oars_plots_deidentified/2026_02_06_oars_2_ph_response_groups_DI.pdf",
...
(((metadata.oars.stool.asv.double.but.i.plot+metadata.oars.fcal.double.plot)/
  ...
  patchwork::plot_layout(heights=c(1,2.2))) %>%
     ggsave(filename="./oars_plots/2026_02_06_oars_2_ph_responders_nonresponders.pdf",
...
# top part
metadata.oars.stool.omics.cor.network.pcoa %>%
  ggsave(filename="./oars_plots/2026_02_25_oars_2_responders_network.pdf",
         width=12, height=5, device = cairo_pdf)

# left part
(oars.fecalcal.omics.cor.plot/patchwork::plot_spacer()) %>%
  ggsave(filename="./oars_plots/2026_02_06_oars_2_responders_fcal_left.pdf",
         width=5, height=9, device = cairo_pdf)

# right part

((patchwork::free(oars.fcal.cor.plot.1,type="label")|oars.fcal.cor.plot.2|oars.fcal.cor.plot.3)/
    ((patchwork::free(plsda_model_plot,type="label")/
        patchwork::free(mg.badol.plot,type="label")/
        oars.badol.simple.plot+patchwork::plot_layout(heights=c(1.2,1.1,0.8)))|
       oars.badol.starch.fcal.plot)+patchwork::plot_layout(heights=c(0.9,4))) %>%
  ggsave(filename="./oars_plots_deidentified/2026_02_06_oars_2_responders_fcal_right_DI.pdf",
...
pheatmap::pheatmap(reshape2::acast(subset(oars.asv.mgx.mpx.mbx.lfc %>% merge(oars.deid) %>% mutate(sample = paste(code, timing)), feature %in% rownames(oars.asv.mgx.mpx.mbx.lfc.map.vimp)),
                   ...
                   breaks=c(seq(min(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc)), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc))/100, max(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc)), length.out=floor(100/2)))) %>%
  ggsave(filename="./oars_plots_deidentified/2026_02_06_oars_2_responders_fcal_heatmap_DI.pdf",
         width=10, height=5)
...
pheatmap::pheatmap(reshape2::acast(subset(oars.asv.mgx.mpx.mbx.lfc, feature %in% rownames(oars.asv.mgx.mpx.mbx.lfc.map.vimp)),
                   ...
                   annotation_colors = list(Response = c(`Strong Response` = gg_color_hue(2)[1],
                                            ...
                                            `Data type` = c(ASV = "#8DD3C7",
                                                            ...
                                                            `Host protein` = "black")),
                   annotation_row = oars.asv.mgx.mpx.mbx.lfc.map.vimp %>%
                     mutate(`Data type` = factor(`Data type`, levels=c("ASV", "Species", "Pathway", "COG", "CAZyme", "Metabolite", "Host protein"))) %>%
                     mutate(`fCal Correlation` = `Fcal Correlation`) %>%
                     dplyr::select(-`Fcal Correlation`),
                   breaks=c(seq(min(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc)), 0, length.out=ceiling(100/2) + 1), 
                            seq(max(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc))/100, max(na.omit(oars.asv.mgx.mpx.mbx.lfc$slfc)), length.out=floor(100/2)))) %>%
  ggsave(filename="./oars_plots/2026_02_06_oars_2_responders_fcal_heatmap_legend.pdf",
         width=10, height=7)
# 
...
(((oars.asv.maaslin.036.volcano/
  ...
  ggsave(filename="./oars_plots/2026_02_06_oars_supp_omics_volcanos.pdf",
...
(oars.asv.data.glom.lmer.volcano+
  ...
  ggsave(filename="./oars_plots/2026_02_06_oars_supp_omics_resp_volcanos.pdf",
         width=18, height=12, device = cairo_pdf)


# Load vs Water
oars.stool.water.load.correlation.plot%>%
  ggsave(filename="./oars_plots/2026_02_06_oars_2b_ph_load_vs_water.pdf",
...
new.fcal = read.csv("~/Downloads/OARS_fCal_df.csv")
new.fcal = new.fcal[,c(1:5)]

new.fcal$OARS.Entry.Fcal = ifelse(is.na(new.fcal$OARS.Entry.Fcal), "", new.fcal$OARS.Entry.Fcal)

new.fcal$OARS.Entry.Fcal = as.numeric(new.fcal$OARS.Entry.Fcal)
      
plot(new.fcal$OARS.Entry.Fcal, 
     new.fcal$REDCap)

cor.test(new.fcal$OARS.Entry.Fcal, 
         new.fcal$REDCap, method="spearman")

metadata.oars.stool[,c("HM", "standard.name", "oars.timing", "fcal")] %>%
  arrange("HM") %>%
  write.csv("~/Downloads/OARS_fCal_PD.csv")

# check original redcap for big errors
# HM0899-STL-03
# HM0899-STL-05
redcap.data.loaded <- read.csv("~/Downloads/redCap_sampleData_uniques_240724.tsv", sep="\t")
subset(redcap.data.loaded, standard.name == "HM0899-STL-03")[,"fecalcal_res"]

subset(redcap.data.loaded, study_id == "HM0899.00")[,c("standard.name", "fecalcal_res")]


# :: Dave's comment about washout -----------------------------------------

# dave asks whether strong fermenters maintain low fCal through washout (check 6M to 9M)
# i.e. are strong fermenters likely to maintain biochemical remission WITHOUT RS

metadata.oars.washout.9m = 
merge(subset(metadata.oars.stool.double, phase == "rs2" & reltiming == "post")[,c("HM", "response", "fcal")],
      subset(metadata.oars.stool, timing == "9M"), by="HM")[,c("HM","timing","response","fcal.x", "fcal.y")]
metadata.oars.washout.9m = metadata.oars.washout.9m %>%
  reshape2::melt() %>%
  mutate(timing = ifelse(variable == "fcal.x", "6M", "9M"))

ggplot(metadata.oars.washout.9m %>%
         mutate(Response = ifelse(response == "high", "High", "Low")),
       aes(x=timing, y=value))+  
  scale_y_log10()+
  geom_boxplot(width=0.5)+
  geom_line(aes(group=HM), linetype=2, alpha=0.5, linewidth=0.5)+
  ...
  labs(x="",
       y="Fecal Calprotectin (μg/g)")
# stats
lmerTest::lmer(log10(value) ~ timing*response + (1|HM), metadata.oars.washout.9m) %>%
  summary() %>% coef()
# interaction p = 0.11
wilcox.test(subset(metadata.oars.washout.9m, response == "high" & timing == "6M")$value,
            subset(metadata.oars.washout.9m, response == "high" & timing == "9M")$value, paired=T)
# p = 0.125      
wilcox.test(subset(metadata.oars.washout.9m, response == "low" & timing == "6M")$value,
            subset(metadata.oars.washout.9m, response == "low" & timing == "9M")$value, paired=T)

# repeat again for 12M

metadata.oars.washout.12m = 
  merge(subset(metadata.oars.stool.double, phase == "rs2" & reltiming == "post")[,c("HM", "response", "fcal")],
        subset(metadata.oars.stool, timing == "12M"), by="HM")[,c("HM","timing","response","fcal.x", "fcal.y")]
metadata.oars.washout.12m = metadata.oars.washout.12m %>%
  reshape2::melt() %>%
  mutate(timing = ifelse(variable == "fcal.x", "6M", "12M"))

ggplot(metadata.oars.washout.12m %>%
         mutate(Response = ifelse(response == "high", "High", "Low")) %>%
         mutate(timing = factor(timing, levels=c("6M", "12M"))),
       aes(x=timing, y=value))+  
  scale_y_log10()+
  geom_boxplot(width=0.5)+
  geom_line(aes(group=HM), linetype=2, alpha=0.5, linewidth=0.5)+
  ...
  labs(x="",
       y="Fecal Calprotectin (μg/g)")
# stats
lmerTest::lmer(log10(value) ~ timing*response + (1|HM), metadata.oars.washout.12m) %>%
  summary() %>% coef()
# interaction p = 0.051
wilcox.test(subset(metadata.oars.washout.12m, response == "high" & timing == "6M")$value,
            subset(metadata.oars.washout.12m, response == "high" & timing == "12M")$value, paired=T)
# p = 0.125      
wilcox.test(subset(metadata.oars.washout.12m, response == "low" & timing == "6M")$value,
            subset(metadata.oars.washout.12m, response == "low" & timing == "12M")$value, paired=T)
...
# stats
oars.washout.912.strong.p = lmerTest::lmer(log10(value) ~ as.numeric(timing) + (1|HM), 
               metadata.oars.washout.6912m %>%
                 subset(response == "high") %>%
                 mutate(timing = factor(timing, levels=c("6M", "9M", "12M")))) %>%
  summary() %>% coef()
# p = 0.036
oars.washout.912.weak.p = lmerTest::lmer(log10(value) ~ as.numeric(timing) + (1|HM), 
               metadata.oars.washout.6912m %>%
                 subset(response == "low") %>%
                 mutate(timing = factor(timing, levels=c("6M", "9M", "12M")))) %>%
  summary() %>% coef()
...
metadata.oars.washout.6912m = 
  merge(subset(metadata.oars.stool.double, phase == "rs2" & reltiming == "post")[,c("HM", "response", "fcal")],
        subset(metadata.oars.stool, timing == "9M"), by="HM")[,c("HM","timing","response","fcal.x", "fcal.y")]
metadata.oars.washout.6912m = merge(metadata.oars.washout.6912m,
                                    subset(metadata.oars.stool, timing == "12M"), by="HM")[,c("HM","response","fcal.x", "fcal.y", "fcal")]
metadata.oars.washout.6912m = metadata.oars.washout.6912m %>%
  reshape2::melt() %>%
  mutate(timing = ifelse(variable == "fcal.x", "6M",
                         ifelse(variable == "fcal.y", "9M", "12M")))
# make df
oars.washout.912.p = data.frame(Response = c("Strong", "Weak"),
                                p = round(c(oars.washout.912.strong.p[2,5],
                                      oars.washout.912.weak.p[2,5]), digits=3))

metadata.oars.washout.6912m.plot = ggplot(metadata.oars.washout.6912m %>%
         mutate(Response = ifelse(response == "high", "Strong", "Weak")) %>%
         mutate(timing = factor(timing, levels=c("6M","9M", "12M"))),
       aes(x=timing, y=value))+ 
  ...
  geom_point(shape=21, fill="grey", size=3)+
  geom_hline(yintercept=250, linetype=1, alpha=1, color="red")+
  geom_text(data = oars.washout.912.p,
            x=2, y=3.5,#(max(na.omit(metadata.oars.washout.6912m$value))), 
            aes(label=paste("𝘱:", p), size=4.5))+
  facet_wrap(~Response)+
  theme_classic()+theme(legend.position="none",
                        plot.title = element_text(hjust = 0.5, size=12),
                        strip.text = element_text(size=12),
...
metadata.oars.washout.6912m.plot %>%
  ggsave(filename="./oars_plots/2026_03_28_oars_supp_6912_fcal_strong_weak.pdf",
         width=6, height=3, device = cairo_pdf)


median(subset(metadata.oars.washout.6912m, timing == "6M" & response == "high")$value)
# 47.5
median(subset(metadata.oars.washout.6912m, timing == "12M" & response == "high")$value)
# 182
